...
> **Phase 3 — SHIPPED (2026-07-22).** Implemented exactly as specified in §6.2/§6.3/§6.5, then fully validated, including a real fresh-install smoke test against the packed tarball.
>
> **Implemented scope.** Four new target-neutral interface files under `src/promise/targets/` (`contract-compiler.ts`: `ContractCompileResult`, `ContractCompiler`; `contract-deployer.ts`: `SlotDeployOutcome`, `DeployedSlotResult` discriminated union, `ContractDeployOptions`, `ContractDeployOutcome` (4 states), `ContractDeployResult`, `ContractDeployer`; `deployment-lookup.ts`: `TargetDeploymentSnapshot`, `DeploymentLookup`; `compiler-verifier.ts`: `DeployedSlotRef`, `TargetVerificationResult`, `TargetCompilerVerifier`) — none export anything from `index.ts`/`standalone.ts`, matching §3.4's confirmed zero-public-API-risk finding. Four new n8n implementations under `src/providers/n8n/` (`capabilities.ts`: `N8N_CAPABILITIES`; `contract-target.ts`: `N8nContractCompiler`, `N8nContractDeployer`; `deployment-lookup.ts`: `N8nDeploymentLookup`; `compiler-verifier.ts`: `N8nCompilerVerifier`), each wrapping existing, unmodified machinery (`compileToPackPlan()`, `PackBuilder.build()`, `N8nApiClient.getWorkflow()`, `verifyCompiledWorkflows()`) rather than reimplementing anything. `src/cli.ts`: three new factories (`resolveContractCompiler()`, `resolveContractDeployer()`, `resolveVerificationTarget()`) placed beside Phase 1's own `resolveN8nApiClient()`, which `resolveVerificationTarget()` now reuses rather than re-reading `N8N_BASE_URL`/`N8N_API_KEY` a second time; `handleContractCompile()` refactored to call all three, with its own pre-existing `{plan, traceability, escalation?}` and `--json` output shapes preserved byte-for-byte (a real external compatibility surface, not just an internal type) via an explicit reshape from `ContractCompileResult`'s own `artifact`-named field back to the CLI's own long-standing `plan` name.
>
> **Two deliberate deviations from the plan's own literal pseudocode, both improvements, both explained here rather than silently made.** (1) §6.2's pseudocode types `ContractCompileResult`/`ContractDeployResult`'s `escalation` field as `CompileEscalationInfo` (imported from `compile.ts`); this implementation instead uses `ContractPreparationEscalation`, imported from `decomposition.ts` -- the plan's own pseudocode pre-dates this session's Phase 2 closeout correction, which established that a target-neutral file under `src/promise/targets/` must have zero dependency, even type-only, on the n8n-specific `compile.ts`. Applying that same, just-established principle to these brand-new neutral files (rather than reintroducing the exact pattern Codex had just corrected) was judged the right call without waiting for a further review round to flag it; `ContractWorkflowTrace` was deliberately left alone, still imported from `compile.ts`, since the plan's own pseudocode specifies exactly that and relocating it was never requested or authorized. (2) `N8N_CAPABILITIES` (§6.1's own pseudocode shows all six `implemented` capabilities as `'supported'` in one combined snapshot) instead marks `executionHistory`/`evidenceExtraction` `'unsupported'` here, since `N8nExecutionHistorySource`/`normalizeN8nExecution()` are genuinely Phase 4 scope and do not exist yet -- §6.1's own guardrail is explicit ("if a field here says 'supported', a matching interface exists and is implemented by that target"), and the plan's combined pseudocode describes the capability declaration once Phases 3-4 have BOTH shipped, not Phase 3 alone. Both will flip to `'supported'` when Phase 4 actually lands.
>
> **New test files (31 new tests across 6 files), each targeting a distinct claim.** `tests/unit/promise/targets/contract-compiler.test.ts` (5): `N8nContractCompiler` is a faithful, escalation-preserving wrapper around `compileToPackPlan()`. `deployment-lookup.test.ts` (3): `N8nDeploymentLookup` fetches by id and guards a mismatched `targetId`. `deploy-outcome-classification.test.ts` (7, the plan's own named file, correction 3's direct test): all three `SlotDeployOutcome` variants and all four `ContractDeployOutcome` variants, via a real `PackBuilder` wrapping a mocked `Kairos.build()` -- including the corrected all-dry-run-produces-`'generated'`-never-`'deployed'` case and a mixed dry-run-with-one-failure case proving `'generated'` requires *every* slot to have generated, not just the absence of a real deploy. `credential-isolation.test.ts` (7, the plan's own named file, correction 1's direct test): structural proof in two layers -- source-text inspection (comment-stripped, to avoid a doc-comment mentioning "N8nApiClient" while explaining its absence from producing a false test failure, a real bug caught and fixed while writing this file) confirming `contract-target.ts` and `resolveContractCompiler()`/`resolveContractDeployer()` never reference `N8nApiClient`/`N8N_BASE_URL`/`N8N_API_KEY`/`resolveN8nApiClient()` at all, plus real runtime runs of both classes with `N8N_BASE_URL`/`N8N_API_KEY` unset confirming nothing throws. `compiler-verifier.test.ts` (4, the plan's own named file, correction 4's direct test): slot-name resolution, fetch errors reported only in `fetchErrors` never merged into `verification.findings`, and the documented indirect-gap conflation itself pinned by a real test (a fetch failure for the one workflow holding a required evidence node produces a `gaps_found` verdict for that requirement, indistinguishable from a genuine gap). `tests/unit/cli/contract-compile.test.ts` (5, new -- this command had no dedicated test file before this phase): real subprocess coverage proving the plan-only path's output shape and exit codes are unchanged, and that `--build --dry-run` fails at the `ANTHROPIC_API_KEY` gate specifically, without ever touching `N8N_BASE_URL`/`N8N_API_KEY` -- the strongest available live-checkpoint substitute in an environment with no Anthropic credentials (below).
>
> **Validation results.** `npm run typecheck`: clean. `npm run lint`: clean. `npm test`: **128/128 test files, 2210/2210 tests passed** -- the file count rose by exactly 6 (122 → 128), the test count by exactly 31, both matching the six new files above precisely; zero regressions anywhere else in the suite, zero modifications to any pre-existing test. `docs-drift.test.ts` passed as part of the suite. `npm run build`: clean. `npm pack`: 275 files (up from Phase 1's 273, matching the eight new bundled modules under `src/promise/targets/` and `src/providers/n8n/` reachable from the `cli.ts` entry point).
>
> **Fresh-install smoke test, run against the real packed tarball, not `tsx`/dev-mode source.** Installed into a scratch project with `@anthropic-ai/sdk` deliberately absent (confirmed). `kairos --help` and `kairos contract compile` (no args, usage text) both ran correctly. A real, full `kairos contract compile <empire-homecare-fixture> --json` run, with `ANTHROPIC_API_KEY`/`N8N_BASE_URL`/`N8N_API_KEY` all explicitly unset, produced the exact expected `{plan, traceability}` shape with the exact three real compiled workflow names ("Referral Intake", "Referral Processing & Outcome Logging", "Referral SLA Escalation") and three traceability entries -- the plan-only path through the new boundary, exercised against the genuine shipped artifact, not a simulation of it. `kairos contract compile <fixture> --build --dry-run --yes`, same three env vars unset, printed exactly `Missing required environment variable: ANTHROPIC_API_KEY` and exited 1 -- confirming, against the real packed binary, that the credential gate reaches `ANTHROPIC_API_KEY` and stops there, never having read either n8n variable.
>
> **Live n8n/Anthropic checkpoint: unavailable in this environment, explicitly still pending, honestly disclosed rather than silently substituted for.** No `ANTHROPIC_API_KEY`/`N8N_BASE_URL`/`N8N_API_KEY` were set anywhere this phase was implemented or validated (confirmed directly, matching every prior phase's own disclosed gap). This means the plan's own §12 checkpoint requirement for this phase -- `kairos contract compile --build --dry-run` and `kairos contract compile --build` against real Anthropic/n8n, confirming the corrected `'generated'`/`'deployed'`/`'partial'`/`'blocked'` overall outcomes live -- could not be run. **What was run instead, and exactly what it does and does not prove:** (1) the fresh-install smoke test above, which is real, unmocked code exercising the real credential gate up to the point a live Anthropic call would begin, on the genuine packed artifact; (2) `deploy-outcome-classification.test.ts`'s unit-level coverage, which exercises the real, unmodified `PackBuilder.build()`/`N8nContractDeployer.deployArtifact()` control flow end-to-end against a mocked `Kairos.build()` (not a mocked `PackBuilder.build()` -- the outcome-classification logic under test is never itself mocked), covering every outcome branch a live run would also exercise. **What this does not, and cannot, prove:** real Anthropic response shapes/quirks, real n8n deployment behavior, or anything about a genuine end-to-end `--build` (non-dry-run) run actually registering and verifying real n8n workflows. That remains exactly what the still-pending live checkpoint is for, named here explicitly rather than implied to already be covered.
>
> **No deviations from the accepted plan's actual behavioral requirements** (the two structural deviations above are both internal-file-organization choices, not behavior changes -- neither is externally observable). Every required invariant was verified, not merely implemented: `IProvider` and the public npm API are untouched (no new file exports from `index.ts`/`standalone.ts`, confirmed by construction -- none of the eight new files are referenced by either entry point); plan-only compile requires no credentials (confirmed live, both in the dev suite and against the packed tarball); `--build --dry-run` may require Anthropic but never n8n -- the packed binary was confirmed live to reach the `ANTHROPIC_API_KEY` gate and exit before ever requesting n8n credentials, and that the deployer itself has no n8n dependency at all is proven structurally (source inspection) and by unit test (`credential-isolation.test.ts`, `deploy-outcome-classification.test.ts`), not by a live, completed dry run -- **a full, successful `--build --dry-run` remains pending**, since no Anthropic credentials were available anywhere this phase was implemented or validated; real deployment constructs n8n verification only after deployed slots exist (the compound gate is unchanged in its real, translated form, confirmed by `credential-isolation.test.ts`'s own gate-ordering assertion); `'generated'`/`'deployed'`/`'partial'`/`'blocked'` outcomes are all correctly classified (7 dedicated tests); registration-drop protection and compiler-verification behavior are unchanged -- **registration construction was mechanically refactored** (it now derives from `deployResult.slots` filtered to `outcome === 'deployed'` instead of `buildResult.workflows` filtered by `workflowId`/`error`), **but the underlying registry persistence (`saveContractWorkflowRegistration()`) and drop-detection (`computeDroppedWorkflows()`) functions themselves were not touched**, only the shape of what is fed into them; fetch errors stay separate from verification findings (dedicated test, plus the indirect-gap conflation itself pinned by test); no target-specific raw value is ever cast inside `cli.ts` (the one cast this arc needs lives inside `N8nCompilerVerifier`, per §6.5's own design); existing n8n behavior is behaviorally equivalent (zero pre-existing test modified, all 2179 pre-Phase-3 tests still pass unchanged). `research/` was not touched at any point.
...
> **Phase 4 — SHIPPED (2026-07-22).** Implemented exactly as specified in §6.4/§6.8, then fully validated, including a real fresh-install smoke test against the packed tarball exercising a full, realistic multi-instance ledger and a real `kairos contract evolve run`.
>
> **Implemented scope.** `src/promise/targets/types.ts` gains `EvidenceFieldItem`/`NormalizedTransitionEvidence`/`NormalizedExecution` (appended to Phase 1's file, per §8's own layout). New `src/promise/targets/execution-history.ts`: `ExecutionHistorySource`, `EvidenceNormalizer`, `assertConsistentTargetIds()`. New `src/providers/n8n/execution-history.ts`: `RawExecutionDetail`, `N8nExecutionHistorySource` (defensive sort/truncate, per correction 9). New `src/providers/n8n/evidence.ts`: `evidenceNodeName()` (relocated from `compile.ts`), `normalizeN8nExecution()` (verbatim runData-parsing logic moved from `ledger.ts`'s own pre-boundary `extractExecutionEvidence()`), `N8nEvidenceNormalizer`. New `src/promise/evidence-extraction.ts`: the target-neutral `extractNormalizedEvidence()` (the pre-boundary extractor's exact decision logic, now operating on an already-normalized `NormalizedExecution`), `buildEntryId()` (ledger identity as `(targetId, id)`, byte-identical for n8n, target-prefixed for any other target), plus `hashCorrelationKeyValue()`/`readCorrelationKeyFromJson()`/`extractFieldsFromJson()`/`buildDetail()` (moved verbatim, operating on the neutral `EvidenceFieldItem.fields` shape instead of raw n8n `item.json`). `compile.ts`'s `evidenceNodeName` is now a re-export from its new home. `ledger.ts`'s `pollWorkflowEvidence()` itself is now target-neutral — it takes an `ExecutionHistorySource`/`EvidenceNormalizer` pair and a `TargetDeploymentRef`, calls `assertConsistentTargetIds()` first, and constructs `PollContractResult`/`ContractPollWatermark` generically (dual-writing the legacy `n8nWorkflowId` alias only when `ref.targetId === 'n8n'`). `ProofLedgerEntry` gains `targetId?: TargetId` (optional, populated on every newly-constructed entry, absent and never fabricated on a legacy one). `AmendmentEvidenceRef` gains `targetId?: TargetId`; all four `kind: 'ledger_entry'`-constructing sites in `evolution.ts` now thread it through from the source `ProofLedgerEntry.targetId` (via a new `toLedgerEntryRefs()` helper for the two direct-`entries`-array sites, and a new `LedgerEntryRef` carrier type for `detectRateHotspot()`'s own `ledgerEntryIds` parameter, which used to be a bare `string[]` with nowhere to carry this information at all); `kind: 'exception_item'` and `kind: 'harness_scenario'` sites are untouched, never given a fabricated `targetId`. `handleLedgerPoll`/`runContractComplianceTick` (`cli.ts`) both now construct one `N8nExecutionHistorySource`/`N8nEvidenceNormalizer` pair before their loop and pass a `TargetDeploymentRef` per registered workflow. `capabilities.ts`'s `executionHistory`/`evidenceExtraction` flip from `'unsupported'` to `'supported'`, now that both are genuinely implemented.
>
> **A necessary, load-bearing design decision, not explicitly spelled out in the plan's own pseudocode: `extractExecutionEvidence()` and `hashCorrelationKeyValue()` remain exported from `ledger.ts`, under these exact names, as backward-compatible facades.** A grep before any code was written found `src/reliability/replay/contract-outcome.ts` and `src/reliability/chaos/contract-outcome.ts` — both explicitly out of scope for this whole arc, per the plan's own §8 non-goals ("No file under `src/reliability/`... is touched") — call `extractExecutionEvidence()` directly with real n8n-shaped `RawExecutionDetail` data from live sandbox executions, and `src/promise/harness.ts` calls `hashCorrelationKeyValue()` directly. Deleting or renaming either function was never an option without touching those out-of-scope files. Both facades now delegate entirely to the new neutral machinery (`normalizeN8nExecution()` + `extractNormalizedEvidence()`) rather than being a second, silently-diverging implementation — confirmed by direct source inspection (`extractExecutionEvidence()`'s own body calls exactly these two functions and nothing else; there is no second parsing path that could coincidentally agree with them). A dedicated test (below) separately checks fixture-level output equivalence between the facade and the direct composition for today's code — a distinct claim from delegation, and not by itself proof of historical pre-refactor parity, which instead rests on `ledger.test.ts`'s own 33 pre-existing tests passing with zero assertion changes.
>
> **One deliberate cleanup, disclosed rather than silently made: `PollableN8nClient` (`ledger-types.ts`) was deleted.** Confirmed by grep before deletion that it had exactly zero remaining consumers once `pollWorkflowEvidence()`'s own signature no longer needed it — its two former test consumers (`ledger.test.ts`, `poll-result-compat.test.ts`) were both migrated to construct real `N8nExecutionHistorySource`/`N8nEvidenceNormalizer` instances instead. Genuinely dead code, not a behavior change.
>
> **Two deliberate deviations from the plan's own literal pseudocode, both internal-file-organization choices, neither a behavior change.** (1) `AmendmentEvidenceRef.targetId?` construction sites thread the source entry's own `targetId` through exactly as-is (via conditional spread) rather than defaulting a missing one to `'n8n'` — never fabricating a value the source data doesn't genuinely carry, symmetric with the plan's own explicit treatment of `kind: 'exception_item'` refs ("never carry a fabricated targetId"). (2) `RawExecutionDetail` lives in `src/providers/n8n/execution-history.ts` rather than `ledger.ts` — `ledger.ts`'s own `extractExecutionEvidence()` facade needs both this type (for its parameter) and `normalizeN8nExecution()` (a value) from the n8n provider layer; defining the type there instead of in `ledger.ts` avoids a genuine two-way circular import between `ledger.ts` and `providers/n8n/evidence.ts` that defining it in `ledger.ts` would have created (`evidence.ts` needing the type back from `ledger.ts`, which needs the function from `evidence.ts`).
>
> **New test files (35 new tests: 34 across 5 new files, plus 1 added to the existing `ledger-store.test.ts`), each targeting a distinct claim from this phase's own required test categories.** `tests/unit/providers/n8n/evidence.test.ts` (8, **normalization**): `normalizeN8nExecution()`'s own contract in isolation — executionRef/eventTime mapping, `sourceItemRef` preserving the exact `run.branch.item` position, transitionEvidence filtered to only transitions with a real matching node, multi-item nodes producing distinct `sourceItemRef`s, empty/missing data handled without a crash. `tests/unit/promise/evidence-extraction.test.ts` (8): 5 fixture-level output-equivalence tests between `extractExecutionEvidence()` (the facade) and `normalizeN8nExecution()`+`extractNormalizedEvidence()` composed by hand — asserted deep-equal (after stripping the one genuinely wall-clock-dependent field, `observedAt` — a real timing race caught while writing this file, described below) — across a complete match, an instance_start case, a skipped case, and an unattributed-correlation-key case, plus a direct assertion every facade-produced entry now carries `targetId: 'n8n'`; and 3 direct, target-neutral `extractNormalizedEvidence()` tests (**normalization** + closeout correction 1) calling it with hand-built `NormalizedExecution` data and a synthetic non-n8n `TargetDeploymentRef`, never touching `normalizeN8nExecution()` at all — proving the `sourceItemRef`-absent array-index fallback for multiple initiating items and for multiple same-transition evidence items (distinct, non-colliding entry ids in both cases), plus a mixed case where some items carry `sourceItemRef` and others fall back to their own array index in the same list. `tests/unit/promise/targets/execution-history.test.ts` (10, **target-consistency** + **ordering/limit**): `assertConsistentTargetIds()`'s pass/throw behavior including a historySource/normalizer mismatch with a matching ref; `N8nExecutionHistorySource` sorting genuinely out-of-order raw API output into newest-first, truncating genuinely over-limit raw output to the requested limit, and guarding a mismatched ref on both `listExecutions()`/`fetchExecution()` without ever calling the underlying client. `tests/unit/promise/targets/ledger-id-generation.test.ts` (3): `buildEntryId()` byte-identical for n8n, target-prefixed and collision-free for a synthetic non-n8n target. `tests/unit/promise/evolution-evidence-ref.test.ts` (5, **evolution-provenance**): `kind: 'ledger_entry'` refs carrying `targetId: 'n8n'` via both the `detectRateHotspot()` path and the direct-`sampleEvidence` path, the identical fixture with no `targetId` on any source entry producing refs with `targetId` genuinely `undefined` (not fabricated), `kind: 'exception_item'` refs never carrying one even when corroborating entries do, `kind: 'harness_scenario'` refs never carrying one either. `tests/unit/promise/ledger-store.test.ts` (+1, **legacy-ledger**): a hand-written legacy-shaped JSONL line (no `targetId` field on disk at all, confirmed by string-checking the raw fixture text itself, not just its parsed shape) read back correctly alongside a freshly-appended new-shaped entry in the same file. **Multi-item** coverage came from `ledger.test.ts`'s own pre-existing "multi-item / multi-run cardinality" suite (5 tests), preserved unchanged and re-verified passing through the new boundary — not duplicated as new tests, since duplicating already-comprehensive, already-passing coverage would have added nothing beyond what re-running the originals through `pollN8n()`/`extractExecutionEvidence()` already proves.
>
> **A real bug caught and fixed while writing the parity test, not a product bug.** `observedAt` is computed via `new Date().toISOString()` fresh inside `extractNormalizedEvidence()` on every call; calling it twice in immediate succession (once via the facade, once via the direct neutral path) legitimately produced a one-millisecond difference on a real run, failing a naive deep-equal comparison. Fixed by stripping only that one wall-clock-dependent field before comparing, then separately asserting both sides produced a real, non-empty `observedAt` of their own — re-run 5 times consecutively to confirm the fix, not just the absence of one failure.
>
> **Extraction-behavior invariants, each independently confirmed, not merely asserted.** `eventTime`/`observedAt`/correlation hashing/multi-item cardinality/skipped-unverifiable-extracted outcome classification/cumulative unattributed counts/gap detection/watermark comparison logic: all 33 of `ledger.test.ts`'s own pre-existing tests (19 for the `extractExecutionEvidence()` facade, 1 for the top-level `hashCorrelationKeyValue` test, 13 for `pollWorkflowEvidence()`) pass **completely unchanged in their own assertions** — only each `pollWorkflowEvidence(...)` call site's function name changed, to a new local `pollN8n()` wrapper that drives the exact same argument list through the real `N8nExecutionHistorySource`/`N8nEvidenceNormalizer` classes rather than a bare mock client interface. `tests/unit/promise/targets/poll-result-compat.test.ts`'s own 2 pre-existing tests (Phase 1's own `PollContractResult` dual-write proof) needed their call sites migrated to the new signature but their assertions are likewise unchanged.
>
> **Validation results.** `npm run typecheck`: clean. `npm run lint`: clean. `npm test`: **133/133 test files, 2245/2245 tests passed** — the file count rose by exactly 5 (128 → 133), the test count by exactly 35, both matching the breakdown above precisely; zero regressions anywhere else in the suite. `docs-drift.test.ts` passed as part of the suite. `npm run build`: clean. `npm pack`: 281 files (up from Phase 3's 275, matching the four new bundled modules — `targets/execution-history.ts`, `providers/n8n/execution-history.ts`, `providers/n8n/evidence.ts`, `promise/evidence-extraction.ts` — reachable from the `cli.ts` entry point, plus their source maps).
>
> **Fresh-install smoke test, run against the real packed tarball, not `tsx`/dev-mode source — the strongest one run in this arc to date, because this phase's own persisted-data shape (`ProofLedgerEntry.targetId`) and cross-module data flow (ledger → evolution) made a genuinely meaningful fixture-based check possible without live credentials.** Installed into a scratch project with `@anthropic-ai/sdk` deliberately absent (confirmed). `kairos --help`, `kairos ledger poll` (usage), `kairos contract evolve run` (usage) all ran correctly. A real contract was imported (`kairos contract import`), then a hand-written, realistic 20-entry ledger (8 drifting + 2 healthy instances, matching this same session's own test fixture shape exactly) was seeded directly to disk with a deliberate, honest split — 16 entries carrying `targetId: 'n8n'` (simulating this phase's own new poll output) and 4 entries with no `targetId` field at all (simulating genuinely legacy, pre-Phase-4 data) — then two real commands were run against the packed binary: `kairos ledger show --json` confirmed all 20 entries round-trip correctly (16 with `targetId`, 4 without, exactly matching what was seeded — legacy entries remain readable, unchanged); `kairos contract evolve run --json` produced 8 real amendment proposals across 5 categories (`sla_threshold_hotspot` ×1, `expiration_rule_hotspot` ×1, `unreached_state` ×4 — one per never-reached state — `unused_transition` ×1, `high_miss_rate` ×1; re-verified directly by re-running this exact smoke test a second time, counting `result.generated.length` programmatically rather than by eye), and **every single one's own `ledger_entry`-kind evidence refs correctly split 16 with `targetId: 'n8n'` / 4 with `targetId` genuinely `undefined`, matching the seeded data exactly, with zero fabrication anywhere** — a packaged-binary/fresh-install smoke-test confirmation of the evolution-provenance requirement against the genuine shipped artifact, not a claim of live operational validation against a real n8n instance (that remains the separately-named, still-pending gap below).
>
> **Live n8n/Anthropic checkpoint: unavailable in this environment, explicitly still pending, honestly disclosed rather than silently substituted for.** No `ANTHROPIC_API_KEY`/`N8N_BASE_URL`/`N8N_API_KEY` were set anywhere this phase was implemented or validated (confirmed directly, matching every prior phase's own disclosed gap). This means the plan's own §12 checkpoint requirement for this phase — a live `kairos ledger poll` against real n8n, including a real batch execution with multiple items per transition, followed by a live `kairos contract evolve run` — could not be run against a real n8n instance. **What was run instead:** the fresh-install smoke test above (real packed binary, real multi-instance ledger data, real evolution-proposal generation, real targetId-provenance verification) plus `ledger.test.ts`'s own pre-existing, still-passing multi-item/multi-run cardinality suite exercising the real, unmodified extraction logic end to end. **What this does not, and cannot, prove:** real n8n execution-list/execution-detail response shapes or quirks, or that `N8nExecutionHistorySource`'s real HTTP calls behave as assumed against a genuine n8n API. That remains exactly what the still-pending live checkpoint is for, named here explicitly rather than implied to already be covered.
>
> **Four closeout corrections made after Phase 4 was first reported complete, before this note reached its final form — documented here because two of them were real, verified counting/proof-scope errors, not stylistic nitpicks.** 1. `extractNormalizedEvidence()` (the target-neutral extractor) had no DIRECT test coverage of its own `sourceItemRef`-absent array-index fallback using a non-n8n `TargetDeploymentRef` — every existing test reached it only indirectly, through the n8n facade/normalizer. Fixed by adding 3 tests to `evidence-extraction.test.ts` that call `extractNormalizedEvidence()` directly with hand-built `NormalizedExecution` data and a synthetic `'in-memory-test'` ref, covering multiple initiating items with no `sourceItemRef`, multiple same-transition evidence items with no `sourceItemRef`, and a mixed present/absent case — proving distinct, non-colliding, target-prefixed entry ids with zero involvement of `normalizeN8nExecution()`. 2. The parity-test doc comment and this note both overstated what the facade-vs-direct-composition test proves — it checks current fixture-level output equivalence, not historical pre-refactor parity (that rests on `ledger.test.ts`'s own 33 unchanged tests) and not delegation by itself (that rests on direct source inspection of `extractExecutionEvidence()`'s own body). Both the test file's own doc comment and this note's wording were corrected to state exactly this, not more. 3. This note's fresh-install smoke-test paragraph claimed "7 real amendment proposals" while its own parenthetical enumeration (`sla_threshold_hotspot`, `expiration_rule_hotspot`, four `unreached_state`, `unused_transition`, `high_miss_rate`) sums to 8 — an arithmetic error in the note, not in the smoke test itself. Verified by re-running the exact same smoke test a second time against a freshly rebuilt tarball, counting `result.generated.length` and `result.generated.map(p => p.category)` programmatically rather than re-reading the original transcript: 8 proposals, 5 categories, confirmed. The note was also reworded to call this a packaged-binary/fresh-install smoke test throughout, not "live" validation — that word is now reserved exclusively for the still-pending, genuinely-live n8n/Anthropic checkpoint named below. 4. `ledger.test.ts`'s own test-count breakdown was stated as "20 for the facade, 14 for `pollWorkflowEvidence()`" — wrong; direct counting (`grep -c` against each `describe` block's own `it(` lines) gives 19 for the facade, 1 for the standalone `hashCorrelationKeyValue` test, 13 for `pollWorkflowEvidence()` — 33 total either way, but the internal breakdown was wrong. Corrected. All four corrections re-validated together: typecheck, lint, the 8 focused Phase 4 test files (94 tests), and the full suite all green — see the updated counts above.
>
> **No deviations from the accepted plan's actual behavioral requirements** (the internal-file-organization deviations above are not behavior changes — neither is externally observable). Every required invariant was verified, not merely implemented: existing n8n extraction behavior is equivalent (33 pre-existing tests, zero assertion changes, plus a dedicated parity test); `eventTime`/`observedAt`/correlation hashing/multi-item cardinality/skipped-unverifiable outcomes/cumulative unattributed counts/gap detection/watermark behavior are all preserved (same evidence as above); old ledger entries without `targetId` remain readable (dedicated test plus a packaged-binary/fresh-install smoke-test confirmation); new ledger entries carry target provenance (dedicated test plus the same packaged-binary/fresh-install smoke-test confirmation); ledger identity is `(targetId, id)`, never `id` alone (`buildEntryId()`'s own target-folding design, dedicated test); execution history is defensively sorted/truncated (dedicated test feeding genuinely out-of-order/over-limit raw input); `normalizeN8nExecution()` performs no ledger-ID generation (confirmed by its own signature returning only `NormalizedExecution`, never touching `ProofLedgerEntry`); `extractNormalizedEvidence()` owns ID generation and the `sourceItemRef`-or-array-index fallback (confirmed by code inspection and the normalization tests above); n8n runData/marker-node names/response shapes never leak into neutral interfaces (`evidence-extraction.ts` has zero reference to `evidenceNodeName`, runData, or any n8n-specific shape, confirmed by direct inspection while writing it); evolution evidence references preserve `targetId` for ledger-derived evidence (dedicated test plus a packaged-binary/fresh-install smoke-test confirmation across all 8 real proposals, 5 categories); SLA compliance/ExceptionDesk/Promise Report/Value Report behavior did not change (none of `sla-compliance.ts`/`exception-desk.ts`/`report.ts`/`value-report.ts` were touched at all this phase, confirmed by the diff itself); existing public npm exports and `IProvider` remain untouched (no new file exports from `index.ts`/`standalone.ts`, confirmed by construction). `research/` was not touched at any point.
...
> **Phase 5 — SHIPPED (2026-07-23).** Implemented exactly as specified in §7, then fully validated. Purely additive: **zero existing product/source/test file was modified this phase; the plan document itself was updated** (this note) — confirmed by `git status` itself, every OTHER change is a new file — the lowest-risk phase of this whole arc by construction, since a test-only reference adapter with no CLI wiring has no production surface to regress.
>
> **Implemented scope.** `src/promise/targets/in-memory/adapter.ts` (new): `IN_MEMORY_CAPABILITIES` (all five implemented capabilities `'supported'`, `compilerVerification` honestly `'unsupported'`, every reliability capability `'unsupported'` — this adapter has no reliability modules at all, not even conditionally); `InMemoryRawExecution` (`{id, startedAt, asNormalizedExecution}` — deliberately trivial, since `seedExecution()` already builds the real, final `NormalizedExecution` up front); `InMemoryContractTarget`, implementing exactly the five specified interfaces (`ContractCompiler<ContractDecomposition>`, `ContractDeployer<ContractDecomposition, ContractDecomposition>`, `DeploymentLookup`, `ExecutionHistorySource<InMemoryRawExecution>`, `EvidenceNormalizer<InMemoryRawExecution>`), never `TargetCompilerVerifier`. `compileContract()` calls the shared `prepareContract()` gate directly — the blocking-assumption/validation check is structurally inherited, not reproduced by hand, confirmed by a dedicated test using the same `empire-homecare-referral-intake-blocking-assumption.json` fixture Phase 2/3's own tests use. `deployArtifact()` generates a fresh `generateUUID()` per slot and stores exactly **one `WorkflowSlot`** per deployment id, in a `Map<string, WorkflowSlot>` — never the whole `ContractDecomposition` — so `fetchDeployment()` structurally cannot return another slot's data (§7's own correction 12, pinned directly by `in-memory-slot-isolation.test.ts` below, not just relied upon implicitly). `listExecutions()`/`fetchExecution()` both guard `ref.targetId`, matching every other target's own adapters.
>
> **Two deliberate deviations from the plan's own literal pseudocode, both disclosed with reasoning, neither a behavior regression.** (1) `seedExecution()`'s real signature is `seedExecution(deploymentId, scenario, contract, now?)`, not the plan's own elided `seedExecution(deploymentId, scenario)` (explicitly marked `/* ... */`, never fully specified in §7 — this method's internals were left for the implementing phase to design). Building a correct `NormalizedExecution` requires resolving `contract.correlationKey.fieldPath` into the same nested shape a real trigger payload would carry it in, which a bare `ContractScenario` cannot supply on its own (it carries only the correlation *value*, never the contract's own field path); `now` is optional, defaulting to real "now," threaded through for the identical deterministic-testing reason `harness.ts`'s own `runScenario(contract, scenario, now)` already takes it. (2) `listExecutions()` genuinely **sorts** by `startedAt` rather than the plan's own literal `.reverse()` on insertion order. A bare reverse only honors the interface's own stated contract ("MUST return executions newest-first... not just usually true," §6.4) if `seedExecution()` is *always* called in strictly oldest-to-newest order — nothing enforces that, and `seedExecution()` iterates a scenario's own timeline array order, not a chronological sort of its own computed timestamps. Sorting instead makes the guarantee structurally true regardless of seed order, exactly mirroring the defensive-sort discipline Phase 4's own `N8nExecutionHistorySource` already established for this identical interface method (correction 9) — a correction for consistency with an already-accepted principle in this same plan, not a new one invented here. Directly tested: `in-memory-adapter.test.ts` seeds three executions in deliberately non-chronological order and confirms `listExecutions()` still returns them newest-first.
>
> **A third design decision, not a pseudocode deviation (the plan left it unspecified): one execution per timeline event, not one execution per scenario.** A `ContractScenario`'s `timeline` can carry multiple events (e.g. `duplicate_correlation`'s two `instance_start` events, or `missing_data`'s `instance_start` + `evidence` pair) — `seedExecution()` appends one `InMemoryRawExecution` per event, matching how a real target has genuinely separate executions for an intake trigger versus a later outcome-update trigger, rather than collapsing an entire scenario's history into one artificial combined execution.
>
> **New test files (28 new tests across 4 files, final count after closeout, verified by direct run).** `tests/unit/promise/targets/in-memory-adapter.test.ts` (15): direct, per-interface coverage at the same rigor Phase 3/4 gave each n8n adapter individually before any cross-target claim was made — `IN_MEMORY_CAPABILITIES`'s own shape, `compileContract()`'s ready/blocked paths via the shared gate, `deployArtifact()`'s unique-id-per-slot and repeated-rebuild behavior plus its real dry-run semantics (closeout correction 1: `'generated'` slots with no `ref`, and a dedicated test proving a dry run leaves nothing fetchable), `fetchDeployment()`'s guard, `listExecutions()`'s genuine sort (non-chronological-seed-order) AND its genuine limit-respecting behavior (closeout correction 2: `limit: 2` against 3 seeded executions, confirming only the newest 2 come back) and both methods' `ref.targetId` guards, `fetchExecution()`'s unknown-id guard, and `normalize()`'s trivial pass-through. `tests/unit/promise/targets/evidence-conformance.test.ts` (7, conformance suite #1, unchanged this closeout — its same-ref design is deliberate, kept): every real, generator-produced scenario for the Empire Homecare fixture (`generateContractScenarios()`, not a hand-picked subset) run through both `normalizeN8nExecution()` (fed a hand-constructed n8n-shaped raw execution) and `InMemoryContractTarget.seedExecution()`+`.normalize()`, then both through `extractNormalizedEvidence()` under the *same* arbitrary `TargetDeploymentRef` — isolating normalization fidelity as the only variable — asserted equivalent. `tests/unit/promise/targets/cross-target-report-parity.test.ts` (3, conformance suite #2, substantially reworked this closeout — see correction 3/4 below): a real n8n ref and a real, different `in-memory-test` ref are each confirmed to produce entries/proposals carrying their own correct provenance, then a full ledger built via both paths across every real scenario is fed through all six named functions (`checkSlaCompliance()`, `updateExceptionDesk()`, `buildPromiseReportData()`, `analyzeContractForAmendments()`, `deriveLearningNotesFromProposals()`, `buildAutomationValueReport()`), each pair asserted both non-vacuous (real, non-empty results) and equivalent once target-specific provenance/ids/timestamps are normalized away. `tests/unit/promise/targets/in-memory-slot-isolation.test.ts` (3, conformance suite #3, correction 12's own direct test, unchanged this closeout): a real multi-slot (3-slot) decomposition deploys independently-fetchable slots whose own `sourceElements` never contain any other slot's; a raw fetch payload is confirmed to be one bare `WorkflowSlot`, never a `{slots: [...]}` container; a simulated rebuild's two generations stay mutually isolated too.
>
> **"Equivalent," precisely defined and never silently loosened into "close enough."** Every real entry/proposal id differs by construction between the two paths — each is built via `buildEntryId()`, which folds in `EvidenceFieldItem.sourceItemRef` when present; n8n's own normalizer always populates it (the exact `run.branch.item` position), the in-memory adapter never does (by design — `sourceItemRef` is documented as "OPTIONAL, opaque, target-provided," and a target that doesn't populate it is expected to fall back to its own array index, not a defect). `evidence-conformance.test.ts` strips only the `id` suffix (verifying instead that both ids are real, well-formed, and reference the correct execution) plus `observedAt` (wall-clock at extraction time). `cross-target-report-parity.test.ts` uses a more general technique for the same underlying reason: a positional id-to-placeholder map built once per path (`<ENTRY_0>`, `<ENTRY_1>`, ...), then a generic recursive redaction pass over each of the six functions' own differently-shaped output — normalizing every embedded id (`ExceptionDeskItem.evidence`, `AmendmentEvidenceRef.id`, and anywhere else one appears) without needing to special-case each function's own structure. Every other field is compared for exact equality.
>
> **A second real timing race found and fixed while writing these tests, not a product bug — distinct from, though the same class as, Phase 4's own closeout finding for `observedAt`.** `evolution.ts`'s own `ContractAmendmentProposal` construction stamps `createdAt: new Date().toISOString()` directly, genuinely wall-clock — confirmed, by design, **not** derived from the `now` parameter `analyzeContractForAmendments()` otherwise takes (a real, correct, pre-existing choice: `createdAt` means "when this record was created," a different concept from the SLA-window analysis instant `now` represents). Calling it twice in a row, once per path, produced a real, observed millisecond-different `createdAt` that failed a naive `toEqual()`. Fixed by blanking both `createdAt` and `observedAt` (defensively — the latter was already pinned to a fixed value before either call, by construction) in the same redaction pass as the id normalization, rather than treating it as a separate special case.
>
> **Validation results (final, after the five closeout corrections above).** `npm run typecheck`: clean. `npm run lint`: clean (this phase's only `src/` change, `adapter.ts`, is fully covered — `eslint.config.js` only lints `src/**/*.ts`, and this phase's test files have no separate lint step, matching every prior phase). `npm test`: **137/137 test files, 2273/2273 tests passed** — the file count rose by exactly 4 (133 → 137) from Phase 4, the test count by exactly 28 (matching the four files' own final counts above precisely); zero regressions anywhere else in the suite, zero existing test modified. `docs-drift.test.ts` passed as part of the suite (no CLI-visible behavior changed — this phase touches no CLI code at all). `npm run build`: clean.
>
> **`npm pack` + fresh-install smoke test: deliberately not run this phase, with reasoning, not silently skipped.** Confirmed directly (`grep -rn "in-memory/adapter" src/`, restricted to non-self matches) that no file under `src/` outside `adapter.ts` itself imports it — none of tsup's five bundle entry points (`index.ts`/`standalone.ts`/`cli.ts`/`mcp-server.ts`/`lint-cli.ts`) reach it through their own import graphs, so it is never bundled into `dist/` at all, exactly like `types.compile-check.ts` was deliberately kept unbundled in Phase 1. Verified, not merely inferred: `grep -rl "InMemoryContractTarget" dist/` after a real `npm run build` returns zero matches. The verified claim is narrower than "the package would be byte-identical" (build tooling can legitimately vary chunk hashes/ordering between runs even with zero semantic change, and that was never independently checked) — it is that the adapter is unreachable from every packaged entry point, so **no packaged-code change is expected** from this phase, and a fresh-install smoke test would therefore be exercising the identical code Phase 4's own smoke test already exercised, surfacing no new information. Per §12's own guidance ("re-run after any phase changing a persisted file shape or CLI-visible behavior") and this session's own standing instruction to run only "appropriate" pack/smoke checks, this phase changes neither, so none was run.
>
> **No live checkpoint gap to disclose this phase.** Unlike every prior phase, Phase 5 needs no Anthropic/n8n credentials at all — it is entirely local, synchronous-or-in-memory-async computation with no network call anywhere in its own code or its own tests. The three conformance suites themselves ARE the checkpoint the plan's own §15 calls for ("Checkpoint: all three suites pass") — run for real, against the genuine, unmodified downstream functions, not simulated.
>
> **Five closeout corrections made after Phase 5 was first reported complete, before this note reached its final form — documented here because the first two were real, verified behavioral gaps, not stylistic nitpicks.** 1. `deployArtifact()` originally ignored `ContractDeployOptions` entirely, always returning `outcome: 'deployed'` regardless of `dryRun` — a real gap given `ContractDeployOptions`/`SlotDeployOutcome` define real dry-run semantics every other target's own deployer honors. Fixed: `dryRun: true` now returns `'generated'` slots with no `ref` at all (overall outcome `'generated'`) and writes nothing into `this.deployments` — structurally nothing for `fetchDeployment()` to find afterward, mirroring n8n's own real "a dry run never registers a fake/placeholder id" behavior. `activate` remains an explicitly documented no-op (an in-memory deployment has no real activation state to toggle). Two new tests in `in-memory-adapter.test.ts` pin this: dry-run slot shape, and a dry-run-then-real-deploy sequence proving the dry run left no fetchable trace at all. 2. The `listExecutions()` "respecting limit" test requested `limit: 20` against only 3 seeded executions — the limit itself was never actually exercised. Fixed: a new test requests `limit: 2` and confirms only the newest 2 of the 3 come back, not just whichever fit. 3. `evidence-conformance.test.ts`'s same-ref design (kept, unchanged, deliberately — it isolates normalization fidelity as the only variable) was, in `cross-target-report-parity.test.ts`, masking a real gap: using one arbitrary, shared ref for both paths meant `targetId`/`sourceWorkflowId` matched by construction, never actually proving each path's own real provenance flows through correctly. Fixed: `cross-target-report-parity.test.ts` now uses a genuine n8n ref for the n8n path and a genuine `in-memory-test` ref for the in-memory path, with a dedicated test asserting every entry's own `targetId`/`sourceWorkflowId` is correct and different per path, and a dedicated assertion that each path's own `AmendmentEvidenceRef{kind:'ledger_entry'}` refs cite the correct, different `targetId` — before the now-necessary redaction pass (which normalizes exactly `targetId`/`sourceWorkflowId`/generated ids/`createdAt`/`observedAt`, nothing else) runs for the business-result comparison itself. 4. Every downstream parity assertion was vacuously satisfiable (`toEqual([])` passes trivially) — no test proved the fixture data actually exercised non-trivial logic. Fixed, verified against real output rather than assumed: this fixture's own 4 real generator scenarios already produce non-empty SLA findings (8), an opened exception (1), report instances (4), and evolution proposals (4) — dedicated `.length).toBeGreaterThan(0)` assertions now pin all four; `buildAutomationValueReport()` already produces 2 real line items even with zero-count instances, by that module's own explicit "a supplied assumption always produces a visible line" design — also now asserted directly. `deriveLearningNotesFromProposals()` alone genuinely returned two empty arrays before this fix, correctly per `learning.ts`'s own design (`deriveDecision()` returns `null` for `status: 'proposed'` — nobody has decided on a freshly-generated proposal yet) — fixed by applying a symmetric accept/reject decision to both paths' proposal sets first, via a pure mirror of `evolution-store.ts`'s own `updateProposalStatus()` data transformation (never its real file I/O), with a fixed decision timestamp introducing no new timing race. 5. This note itself overstated two claims: "zero existing tracked file was modified" was false (the plan document, a tracked file, was modified by this very note) — corrected to name product/source/test files specifically, with the plan document's own update named separately; and "a `npm pack` would produce a byte-identical package" was unverified (build tooling can legitimately vary chunk hashes/ordering run to run even with zero semantic change) — corrected to the narrower, actually-verified claim: the adapter is unreachable from every packaged entry point, so no packaged-code change is expected. All five corrections re-validated together: typecheck, lint, the four focused Phase 5 test files (28 tests after these fixes, verified by direct count, not computed by hand), and the full suite all green — see the updated counts above.
>
> **No deviations from the accepted plan's actual behavioral requirements** (the two structural pseudocode deviations above, plus the one-execution-per-event design decision, are implementation choices within a deliberately underspecified method, not departures from any stated invariant). Every required invariant was verified, not merely implemented: all five specified interfaces are implemented, `TargetCompilerVerifier` is not (confirmed by the class declaration itself, `compilerVerification: 'unsupported'`); deployment data is stored and fetched per individual slot, never the whole decomposition (dedicated slot-isolation suite, 3 tests); dry-run semantics are honored for real, never silently ignored (dedicated tests, closeout correction 1 above); the shared `prepareContract()` gate is reused, not reimplemented (dedicated test using the real blocking-assumption fixture); capabilities are declared honestly (dedicated test asserting the exact five-supported/one-unsupported/all-reliability-unsupported shape); the adapter is test-only with no production runtime, CLI target selection, persistence, network, or credential surface (confirmed structurally — zero disk I/O, zero network call, zero CLI wiring anywhere in `adapter.ts`, confirmed by direct inspection while writing it, and zero `--target` flag exists anywhere in `cli.ts`); equivalent normalized evidence from two genuinely DIFFERENT target refs produces equivalent, non-vacuous SLA compliance/ExceptionDesk/Promise Report/Contract Evolution/Learning notes/Value Report results, with each path's own provenance separately confirmed correct before being normalized away for the business-result comparison (closeout correction 3 above); existing n8n behavior is unmodified (zero existing product/source/test file touched this phase — only this plan document itself was updated, confirmed by `git status` itself). `research/` was not touched at any point.
