...
## Pre-audit resolution (2026-07-08, before Step 3 proper)

Both open items above were investigated read-only, no fixes applied.

**Rule 126 (item 2 above) — real overclaim in the warning message, not a functional defect.** Checked `checkRule126` (`src/validation/validator.ts`, ~line 3267): it flags any node `id` that doesn't match a strict UUID v4 regex, with message text stating *"n8n requires UUID v4 format ... for all node IDs."* Checked against real n8n source: `packages/workflow/src/workflow-structure-validation.ts`'s Zod schema defines a node's `id` as `z.string().optional()` — **no format constraint at n8n's own structural-validation layer.** Separately confirmed n8n's editor frontend (`nodeCreator.utils.ts` and others) does generate `uuidv4()` IDs by convention when a human adds a node via the canvas — so UUID v4 is a real, dominant *convention*, but not a requirement n8n itself enforces or rejects violations of. No evidence found of a runtime dependency (execution engine, DB entity) on ID format specifically. **Conclusion: the rule's underlying check may still have some value (consistency with n8n's own convention, avoiding confusion), but its message currently overclaims a hard requirement that doesn't exist — this is why it fires on nearly every generated node (2,263 times) rather than genuinely broken output.** This is a confidence-filtered-reporting candidate (soften the message, and/or reconsider WARN prominence), not a Step 4 validator-logic fix, and it is **not** part of Step 3's scope (Step 3 is Rule 58/node-syncer/registry/typeVersion only) — noted here, left untouched, no code changed.

**`warnedRules` semantics (item 5 above) — resolved, not a bug.** Traced the full path: `BuildCompleteData.warnedRules` (`client.ts`) ← `designResult.warnedRules` ← `WorkflowDesigner.design()`'s return value (`designer.ts:177`) ← `this.promptBuilder.getWarnedRules()` ← `PromptBuilder.getActivePatterns()` (`prompt-builder.ts:214-228`), which returns the rule numbers of the pattern-learning system's currently-active patterns (top-N by composite score, excluding `resolved`/`pending_review`) — **the rules the generation prompt was proactively warned about *before* generation started, based on historical failure-rate telemetry.** This is computed **independently of whether those rules actually fired as issues in this specific build's output.** So Rule 17/14 at 357 occurrences and Rule 90/66 at 199/157 do not mean those builds' delivered workflows contained those errors — they mean those rule numbers were part of the proactively-injected pattern guidance for that many builds. **No bug. The field name is genuinely ambiguous** (easy to misread as "issues present in the final delivered workflow," which is the misreading in item 5 above) — a candidate for a future rename (e.g. `promptGuardedRules`) as minor polish, not a Step 3/4 correctness item. No code changed.
