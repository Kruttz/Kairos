...
**⚠️ v4.1 correction (post-implementation) — workflow lifecycle semantics and a backward-compatibility overclaim, found after Step 8 shipped:**
1. **`deployed` conflated "exists in n8n" with "live."** `WorkflowReference.deployed` only ever meant `!result.dryRun` -- true the instant a workflow was deployed, with no way to tell "deployed but not yet activated" from "activated." New `activated: boolean` field (`deployed && !result.activationRequired` -- reliable because a real activation failure throws `DeployActivationError` before a `BuildResult` is ever returned, so reaching this point with `activationRequired: false` already means activation genuinely succeeded).
2. **`webhookUrl` was never precisely defined, and the surrounding prose implied more than it should have.** It is the *deterministic production endpoint* -- constructed the same way `N8nApiClient.triggerWebhookProduction()` constructs it -- not proof the workflow is active, its webhook is registered, or the endpoint is reachable or callable. It populates once a workflow is deployed (state 2 below), independent of activation, since the URL string itself doesn't depend on activation state.
3. **Three lifecycle states must render distinctly**, not the previous two: (1) dry-run -- not created, relative method/path only, no URL; (2) deployed but inactive -- URL known (the deterministic endpoint), activation still required; (3) activated -- URL known, but reachability remains unverified unless an explicit reachability probe ran (new optional `webhookVerified?: boolean | null` field, mirroring `BuildResult.webhookVerification.reachable` exactly -- absent means "never checked," not "confirmed unreachable"). `renderPriorContext()`'s rendering and `WorkflowReference`'s own doc comments both corrected to state all three explicitly, in `src/pack/workflow-reference.ts` and `src/generation/prompt-builder.ts`.
4. **The "byte-for-byte backward compatible" claim was an overclaim.** *Generation behavior* (prompt, cost, model output, result order) is genuinely unchanged for a dependency-free plan -- that claim stands. But every `PackWorkflowResult` (§12) now additively carries `workflowKey`/`dependsOn` regardless of dependency count, so the *serialized result shape* is not byte-identical to a pre-chaining pack. Both facts are real; §10 and the Guardrails/Process-9 sections below now state both instead of only the stronger one.

Landed as its own closure commit in Step 8 (not a design-only change this time, since the code had already shipped) -- `WorkflowReference` gained `activated`/`webhookVerified`, `toWorkflowReference()` computes them, `renderPriorContext()` renders all three states with dedicated tests for each, and the backward-compatibility wording was corrected everywhere it appeared in this document.
...
### 4. What does a "prior output" contain? (v4.1: `activated`/`webhookVerified` added, correcting a lifecycle-semantics gap)
...
interface WorkflowReference {
   ...
   * "build failed" (blocks downstream, see §7). deployed alone does NOT mean live -- see
   * `activated`. */
  deployed: boolean
  /** True only when deployed AND activation was requested and succeeded (sourced from
   * BuildResult.activationRequired -- reliable because a real activation failure throws
   * before a BuildResult is ever returned). A deployed-but-not-activated workflow still has
   * a real webhookUrl; it just won't respond to requests yet. */
  activated: boolean
  /** Set only when the upstream build ran an explicit reachability probe (mirrors
   * BuildResult.webhookVerification.reachable exactly). Absent means "never checked," never
   * "confirmed unreachable" -- most builds never run this probe. */
  webhookVerified?: boolean | null
  workflowId: string | null
  httpMethod?: string
  webhookPath?: string
  /** The deterministic n8n production endpoint URL -- NOT proof the workflow is active, its
   * webhook is registered, or the endpoint is reachable/callable. Populates once deployed and
   * a base URL is known, independent of `activated` (state 2 below). */
  webhookUrl?: string
  nodeNames: string[]
...
Three lifecycle states this type must represent distinctly (rendered in §6's prompt section):
1. **Dry-run** -- `deployed: false`, `activated: false`. Nothing created in n8n; only relative `httpMethod`/`webhookPath` known, never a URL.
2. **Deployed but inactive** -- `deployed: true`, `activated: false`. `webhookUrl` is a real, deterministically-constructed endpoint, but the workflow hasn't been activated -- it will not actually respond yet.
3. **Activated** -- `deployed: true`, `activated: true`. `webhookUrl` is still just the deterministic endpoint; whether it's actually reachable is the separate `webhookVerified` fact, present only when a real reachability probe ran.
...
### 5. The method/path/full-URL distinction, and one shared URL builder (locked from v3; v4 adds the shared-builder requirement; v4.1 corrects what `webhookUrl` claims to mean)

`webhookPath` (e.g. `"/intake"`) is a relative path fragment, never a dereferenceable URL on its own. `webhookUrl` may only be populated when `N8N_BASE_URL` is actually known/configured at build time — **never fabricated or guessed when the base URL is unknown**.

**v4.1 correction:** `webhookUrl` is the **deterministic production endpoint**, not proof of anything beyond that string being correctly constructed. It is **not** proof the workflow is active, that its webhook is registered with n8n, or that the endpoint is reachable or callable right now -- those are the separate `activated`/`webhookVerified` facts (§4). It populates once the workflow is deployed and a path is known, regardless of activation state, since the URL string itself doesn't depend on whether the workflow has been activated yet.

**v4 addition (unchanged):** `webhookUrl` must be constructed by calling the **exact same logic** `N8nApiClient.triggerWebhookProduction()` already uses, extracted into one small shared pure function — `buildWebhookUrl(baseUrl: string, path: string): string` in `src/utils/webhook-url.ts` — called from both `triggerWebhookProduction()` and `toWorkflowReference()`. Not reimplemented, not approximated: the same trailing-slash-stripping, same leading-slash-ensuring, same `/webhook` segment. Two independent implementations of the same URL-join is exactly the kind of duplication that silently drifts apart over time.

The prompt-builder rendering (§6) must render method/path/URL as clearly distinct labeled fields, **and** distinctly render all three lifecycle states from §4 — never letting the presence of a URL alone imply the workflow is live or reachable.

### 6. How does it reach the next `build()` call? (locked, unchanged from v2/v3; v4.1 corrects the rendering to distinguish three states, not two)

Extend `BuildOptions` (currently `{ dryRun?, activate?, name?, smokeTest? }`) with `priorContext?: WorkflowReference[]`. Extend `src/generation/prompt-builder.ts` to render these as a small, clearly-labeled, deterministic section (e.g. "Related workflows already built in this pack: ...") with method/path/URL rendered as separately labeled lines. **v4.1:** the per-reference header renders one of §4's three lifecycle states explicitly -- "not created" (dry-run), "deployed, activation still required" (state 2), or "activated, reachability [verified / not responding / inconclusive / not verified]" (state 3, keyed off `webhookVerified`) -- rather than the earlier two-state "(not yet deployed)"-or-nothing distinction, which couldn't represent "deployed but inactive" separately from "live." Never a free-form LLM-summarized version — the next generation call sees exactly and only what was structurally extracted.
...
### 10. Backward compatibility (locked, unchanged from v2/v3; now precisely scoped by §3 Pass 1, and the claim itself corrected in v4.1)

A plan whose workflows all have `dependsOn` **absent** (not present-and-empty, not present-and-malformed — genuinely absent, per §3 Pass 1) produces an empty dependency graph: `resolveBuildOrder()` returns the plan's own order (trivially, since an empty graph's topological sort is just insertion order), no workflow is ever rejected or gated, `priorContext` is never populated, and the rendered prompt has no new section. **Generated prompts and generation behavior are unchanged** -- same prompt, same cost, same model output, same build order.

**Corrected (v4.1): this is not "byte-for-byte" at the result-object level, and the plan should stop claiming it is.** Every `PackWorkflowResult` -- built or rejected, dependency-free or not -- now additionally carries `workflowKey` and `dependsOn` (§12, persistence). A zero-dependency workflow gets `workflowKey: 'some-slug'` and `dependsOn: []` where a pre-chaining pack would have had neither field at all. The *behavior* a caller observes (what gets built, in what order it's returned, what it costs) is unchanged; the *serialized shape* of the result is not identical to what a pre-chaining version of Kairos would have produced. Both facts are true and belong in this document; asserting only the stronger one was an overclaim.
...
- Zero generation-behavior change for packs whose plan declares no dependencies (`dependsOn` genuinely absent, §3 Pass 1) — same prompt, same cost, same model output, same result order. **Not** the same claim as "byte-for-byte identical result objects" — `workflowKey`/`dependsOn` are additive fields present on every `PackWorkflowResult` regardless of whether that workflow has any dependencies (§10).
...
9. A new golden pack fixture specifically exercising chaining — two workflows where the second `dependsOn` the first — asserting the second's generated JSON references the first's **real** webhook path/method (not guessed/hallucinated); a fixture confirming a pack with no declared dependencies has unchanged *generation behavior* (same prompt, cost, model output, result order) as before this step -- **not** byte-for-byte identical result objects, since `workflowKey`/`dependsOn` are now additive fields on every `PackWorkflowResult` regardless of dependency count (§10); a fixture confirming an unknown/ambiguous/malformed/self/cyclic declaration rejects only the affected workflow, with the rest of the pack unaffected; a fixture confirming a duplicate declaration builds normally against the deduped edge set; a fixture confirming a build-time failure cascades to skip its dependents; a fixture confirming a fully-dry-run chained pack builds every workflow normally with `deployed: false` throughout.
10. **(v4.1 closure commit, added after a second review of commits 1-9.)** `WorkflowReference` gains `activated`/`webhookVerified`; `toWorkflowReference()` sources `activated` from `BuildResult.activationRequired`; `prompt-builder.ts`'s rendering is rewritten to distinguish all three lifecycle states from §4 instead of commit 6's binary one; this document's "byte-for-byte backward compatible" claim is corrected everywhere it appeared (§10, Guardrails, commit 9 above) to separate "generation behavior unchanged" (true) from "serialized result shape byte-identical" (false). Unit tests added for all three states plus the four `webhookVerified` variants (true/false/null/absent).

**Where:** Same files as Step 7 v4, plus `tests/` for the new chaining fixture(s).

**Reasoning:** Ten small commits behind fixtures (not one large commit) means any regression is caught and attributable to a specific small change — consistent with the one-commit-per-item discipline the Delivery Bundle and preflight arcs already established as Kairos's working pattern. `buildWebhookUrl()` lands first because `toWorkflowReference()` depends on it, and extracting existing logic is lower-risk than writing new logic — proving the extraction is byte-for-byte equivalent to the inline version before building anything new on top of it (this specific claim is about the URL-builder extraction, not the pack-result backward-compat claim commit 10 corrects). `resolveBuildOrder()` lands early because everything downstream depends on its correctness, and it's the piece most amenable to thorough, fast, isolated unit testing before any LLM-dependent behavior is layered on top. Commit 8 intentionally bundles three design sections (§§7/11/12) that v4 explicitly identified as sharing one loop body — splitting them into separate commits would mean each intermediate commit temporarily breaks one of the other two's invariants. Commit 10 lands separately from 1-9 because it corrects semantics a second review found only after 1-9 already shipped, not because it was planned alongside them.
...
- [x] Step 9 (2026-07-09, `docs/plans/step9-checkpoint-2026-07-09.md`): full `plan()→build()→writeBundle()→preflight` pipeline run against the real Anthropic API and a real n8n Cloud account. Chaining validated against genuine (non-mocked) model output -- a dependent workflow's generated content correctly referenced its dependency's real deployed webhook URL. Blocking-assumption activation-suppression verified live (7-workflow pack, all deployed, none activated). Provenance/bundle/preflight/ledger all confirmed correct. Activation path tested separately via a hand-authored, side-effect-free two-workflow pack; reproduced a known, already-documented n8n platform gap (`active: true` without webhook registration) and confirmed Kairos's existing `webhookVerification`/`webhookVerified` handling is the correct, already-built response to it, not a new gap. Two real bugs found and fixed: `PackBuilder.plan()`'s hardcoded `max_tokens: 4096` (truncated on a realistic prompt) and `derivePackName()`'s uncapped slug (`ENAMETOOLONG` on a realistic business description) -- both in their own small commit. One product-level finding recorded, not acted on: `PLAN_PROMPT` hardcodes a 4-8 workflow range that overrides explicit smaller-pack requests.
