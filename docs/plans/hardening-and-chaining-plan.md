...
**Amendment history:** v1 (initial). v2 — incorporated a corrections pass covering: deterministic no-network fixtures with no live Anthropic calls in required CI, no committed failing tests, chaining dependency validation (unknown/forward/cycle handling), stable workflow dependency keys instead of display names, method/path/full-URL distinction in `WorkflowReference`, provenance hashing extended to connections/settings, content-derived (not manually-bumped) version fields, and a tiny non-blocking ledger v0 so Step 6 cannot delay chaining or Empire Homecare. v3 (this revision) — Steps 1–6a executed and closed (including two Codex review rounds on Step 5's provenance work that found and fixed real gaps, and a best-effort fix on Step 6a's telemetry emissions); Step 7's design tightened before any Step 8 code: unknown/self-dependency/cyclic dependency declarations now rejected per-workflow before generation spend (not v2's graceful degrade) — duplicate dependencies, on manual trace-through, turned out to warrant a different, lighter response (dedupe-and-build, not reject; see §3) — a new build-time cascading-unavailability gate distinct from plan-time rejection, and an explicit dry-run-vs-unavailable distinction so dry-run packs (the common case) don't lose chaining.
...
## 9. Step 7 — Pack-builder output chaining: DESIGN (locked, v3)
...
**⚠️ v3 correction (this revision) — failure semantics tightened, superseding v2's softer language:** v2 designed every validation failure (unknown dependency, cycle) as *graceful degradation*: surface a warning, build the workflow anyway without that one chain. A second review round asked for stricter, explicit guarantees this document had not yet locked: **(a)** unknown/duplicate/self/cyclic dependency declarations must be *rejected before any generation spend*, not merely warned about while still building; **(b)** a downstream workflow must not build or activate when a *validly-declared* dependency's reference is genuinely unavailable (the upstream build failed), not just chained without it; **(c)** dry-run packs — which never deploy anything — must not trip that same gate, since "never deployed" and "failed to build" are different conditions that a naive null-check would conflate. All three are locked below — with one refinement found while doing so: manually tracing a concrete duplicate-dependency case (§3) showed that treating a redundant-but-otherwise-valid duplicate identically to a genuinely broken unknown/self/cyclic declaration would reject a perfectly buildable workflow over a harmless LLM quirk. Duplicates are detected and deduped, not rejected — the other three are rejected exactly as instructed. This is exactly the kind of thing a design-lock step exists to catch before implementation starts, not after.
...
**Confirmed before writing this revision:** `PackBuilder.build(plan, options)`'s `dryRun`/`activate` are pack-wide, applied identically to every workflow in the loop (`src/pack/pack-builder.ts` lines ~209–219) — there is no existing per-workflow dry-run mixing. This matters directly for the dry-run-honesty guarantee below: within one pack build, either every workflow is dry-run or none are, so "upstream was dry-run" is a pack-wide, not per-workflow, condition to detect.

**How — the design must answer precisely:**

### 1. How are workflows keyed? (locked, unchanged from v2)
...
### 2. How are dependencies declared? (locked, unchanged from v2)

Extend `WorkflowPlan` with an optional `dependsOn?: string[]`. Extend `PLAN_PROMPT` to explicitly ask the planning LLM to declare, per workflow, which other workflows it needs to reference, with a worked example matching the prompt's existing per-field style. The model naturally thinks in workflow *names*, not slugs it's never told about — so the prompt asks for `dependsOn` as names, resolved to `workflowKey`s during the same normalization pass that assigns keys. The LLM never computes or reasons about slugs.

### 3. Dependency-graph validation — REJECTED before generation spend (tightened in v3)

After `plan()` returns and workflows/keys/`dependsOn` are normalized, validate the declared dependency graph before the build loop runs at all — this validation is a pure function, zero Anthropic calls, so by construction nothing here can ever cost generation spend regardless of outcome. Four categories checked; **three are structural rejections, one is a detected-and-corrected redundancy** — verified by manually tracing a concrete duplicate-dependency case while locking this design, which showed treating all four identically would be disproportionate (see below):

- **Unknown dependency (rejects the workflow):** a `dependsOn` entry that doesn't resolve to any `workflowKey` in the plan (hallucinated name, or a typo that survived normalization). No valid edge can be constructed — there is nothing to build against.
- **Self-dependency (rejects the workflow):** a workflow's `dependsOn` lists its own `workflowKey`. Structurally a 1-node cycle, but called out as its own explicit test case rather than silently subsumed under "cycle" — self-loops are a classic off-by-one source in naive graph-traversal code, so the implementation must prove it explicitly, not rely on the general cycle detector happening to also catch it.
- **Cycle (rejects every workflow in the cycle):** standard DFS-based cycle detection over the `dependsOn` graph (self-dependency is the degenerate 1-node case of this). No valid linear build order exists for the workflows involved.
- **Duplicate dependency (deduped, workflow still builds):** the same `workflowKey` listed more than once in one workflow's `dependsOn`. Unlike the three above, this is not structurally broken — deduping to a single edge produces a perfectly valid dependency set. Rejecting the workflow over redundant-but-otherwise-fine input would be a disproportionate response to a benign LLM quirk, not a real safety property. **Detected and surfaced** (recorded in the same `rejected`-adjacent output as an advisory entry, so it's visible to whoever inspects the plan/build output) **but the workflow builds normally** against the deduplicated edge set — this is the one category where "caught before generation spend" means "the redundancy never silently propagates unexamined," not "the workflow is blocked."

**v3 semantics (supersedes v2) for the three genuine rejections:** a workflow whose `dependsOn` contains an unknown dependency, a self-dependency, or participates in a cycle is **rejected individually** — it does not build, and does not spend a single generation token. It gets a `PackWorkflowResult` with `error` set (reusing the exact shape already used for a thrown build exception — this is conceptually the same kind of per-workflow failure, just caught before generation instead of during it), `deployed: false`, `workflowId: null`. **This rejection is scoped to that one workflow, not the whole pack** — an unrelated workflow elsewhere in the same pack with no dependency problems still builds normally. (v2 had this as a soft warning with degraded-but-successful building; that is no longer the behavior for these three — the plan-level `TypedAssumption`/`EscalationInfo` channel is **not** used here, since that channel is pack-wide/all-or-nothing by design, and this rejection is deliberately per-workflow.)

**Forward dependency (the ordering problem, unchanged from v2):** since a workflow can only chain from something already built, **do not build in the LLM's listed plan order** — build in a topologically-sorted order derived from the validated (acyclic, resolved) dependency graph instead. A workflow that depends on another always builds after it, regardless of which order the plan happened to list them in. This makes "forward dependency" a non-issue by construction — the sort itself is what resolves it, so a plan that lists a dependency before its declaration is exactly as valid as one that lists it after, as long as the resulting graph is acyclic.

This validation is one small, independently unit-testable pure function: `resolveBuildOrder(workflows: WorkflowPlan[]): { order: WorkflowPlan[]; rejected: Map<string, DependencyRejection[]>; deduped: Map<string, string[]> }`, where `order` contains every workflow (rejected ones too — rejection means "don't chain," not "don't return it from this function"; the build loop is what actually skips generation for a rejected key), `rejected` maps a `workflowKey` to the specific structural problem(s) found (unknown/self/cycle only), and `deduped` maps a `workflowKey` to any duplicate entries that were collapsed (advisory only, never blocks).

### 4. What does a "prior output" contain? (locked, with one v3 addition)
...
interface WorkflowReference {
   ...
   * time) -- see the dry-run-honesty section below. When false, workflowId/webhookUrl are
   * always null/absent; this is the field that distinguishes "never deployed" (fine, expected
   * in a dry-run pack) from "build failed" (blocks downstream, see section 7). */
  deployed: boolean
...
### 5. The method/path/full-URL distinction — a real safety catch (locked, unchanged from v2)

`webhookPath` (e.g. `"/intake"`) is a relative path fragment, never a dereferenceable URL on its own — a model that treats it as one could generate a broken or nonsensical reference. `webhookUrl` (a complete, callable URL) may only be populated when `N8N_BASE_URL` is actually known/configured at build time — **never fabricated or guessed when the base URL is unknown**. The prompt-builder rendering (section 6) must render these as clearly distinct labeled fields — "HTTP method," "webhook path (relative)," and, only when present, "full webhook URL" — so the model is never left to infer which kind of reference it's looking at.

### 6. How does it reach the next `build()` call? (locked, unchanged from v2)

Extend `BuildOptions` (currently `{ dryRun?, activate?, name?, smokeTest? }`) with `priorContext?: WorkflowReference[]`. Extend `src/generation/prompt-builder.ts` to render these as a small, clearly-labeled, deterministic section (e.g. "Related workflows already built in this pack: ...") with method/path/URL rendered as separately labeled lines, and `deployed: false` entries rendered with an explicit "(not yet deployed — path/method only, no live URL)" caveat rather than silently omitting the field and leaving the model to wonder. Never a free-form LLM-summarized version — the next generation call sees exactly and only what was structurally extracted.

### 7. Reference availability gating — build-time, distinct from section 3's plan-time rejection (new in v3, resolves the guarantee this document had not yet locked)

Section 3 rejects a *declaration* before any building starts (free, offline, no generation spend at risk by construction). This section is about a **different failure window**: a dependency that passed validation (real `workflowKey`, acyclic) whose upstream workflow's *actual build* then fails for an unrelated reason — a real `Kairos.build()` exception, or the upstream workflow was itself transitively rejected/failed. This can only be known after building has started, so it cannot be caught by section 3's offline validation.

**v3 semantics:** using the build order from section 3, after each workflow builds (or is rejected/fails), record its outcome in a `Map<string, WorkflowReference | 'unavailable'>` keyed by `workflowKey` (`'unavailable'` covers both "rejected in section 3" and "build threw"). Before building the next workflow:

- Look up each of its (validated) `dependsOn` keys.
- If **every** dependency resolved to a real `WorkflowReference`, build normally with `priorContext` populated.
- If **any** dependency resolved to `'unavailable'`, **do not build this workflow at all** — no generation spend, `PackWorkflowResult.error` set naming which upstream dependency was unavailable and why (rejected vs. build failure), `deployed: false`. This is the literal "downstream workflows do not build or activate when required upstream references are unavailable" guarantee, and it **cascades**: if C depends on B depends on A, and A fails, B is marked unavailable too (never attempted), and C sees B as unavailable and is never attempted either. A direct-dependency-only design (section 8) still needs this cascade, since "which workflows are affected" is a transitive question even though "what context gets injected" is not.
- This still never hard-fails the *whole pack* — an unrelated workflow with no path back to the failure builds normally. Only the dependent subgraph is affected.

**The critical distinction this section exists to make explicit:** `deployed: false` (dry-run, section 4) is **not** the same condition as `'unavailable'` (section 7). A dry-run upstream workflow is fully available for chaining — its `WorkflowReference` exists, `deployed` is honestly `false`, and downstream workflows build against it normally (see section 8). Only an upstream workflow that was *rejected* (section 3) or whose *build itself threw* counts as `'unavailable'`. Conflating these two would make chaining useless for the most common real-world case — a dry-run preview of a whole pack before committing to any real deploy.

### 8. Dry-run chaining reports unresolved deploy-time references honestly, never blocks on them (new in v3)

Since `dryRun` is pack-wide (confirmed above), a dry-run pack means every workflow's `WorkflowReference.deployed` is `false` and every `workflowId`/`webhookUrl` is null/absent throughout the whole chain — this is expected, not a failure, and section 7's availability gate must never trigger on it. What a dry-run pack *does* get, fully populated: `workflowKey`, `workflowName`, `httpMethod`, `webhookPath`, `nodeNames`, `credentialsUsed` — everything derivable from generated JSON alone, with no live n8n dependency. A downstream workflow in a dry-run pack chains against its dependencies' paths/methods/node names exactly as it would in a live build; it just never sees a `workflowId` or a callable `webhookUrl`, and the rendered prompt section (section 6) says so explicitly rather than silently omitting the field.

Net effect: dry-run packs get full **content-level** chaining (what the upstream workflow does, its trigger shape, its credentials) with **zero deploy-time** data (nothing to reference by ID or URL, because nothing was deployed) — an honest, useful degradation, not a blocked one.

### 9. Token-budget guardrail (locked, unchanged from v2)

Cap `priorContext` to directly-declared dependencies only (no transitive chains in v1 — section 7's cascade is about *which workflows get blocked*, not about injecting transitive context into a workflow's prompt). Keep each `WorkflowReference`'s rendered form compact — node *names*, not node objects; path/URL strings, not the trigger node object. Never inline a full `N8nWorkflow` JSON object into a subsequent prompt.

### 10. Backward compatibility (locked, unchanged from v2)

A plan whose workflows declare no `dependsOn` at all — the common case today, and every pack ever built before this feature existed — produces an empty dependency graph: `resolveBuildOrder()` returns the plan's own order unchanged, no workflow is ever rejected or gated, `priorContext` is never populated, and the rendered prompt has no new section. Same prompt, same cost, same output, same build order as today. This needs its own explicit test (section under Process below), not just an assertion in prose.

**Where (design touches, implemented in Step 8):** `src/pack/pack-builder.ts` (`WorkflowPlan` + `workflowKey` assignment, `PLAN_PROMPT`, the build loop reordered by build order and gated per section 7), `src/types/options.ts` (`BuildOptions`), `src/generation/prompt-builder.ts` (rendering), new `src/pack/workflow-reference.ts` (the `WorkflowReference` type + a pure `toWorkflowReference(result: BuildResult, n8nBaseUrl?: string): WorkflowReference` reusing `findWebhookTrigger()`), new `src/pack/dependency-graph.ts` (the `resolveBuildOrder()` pure function + cycle/duplicate/self/unknown detection), `src/pack/pack-bundle.ts` (import site of the existing `slugifyWorkflowName`).

**Reasoning:** The earlier assumption that a wiring graph already existed would have led straight to a design bug if implementation had started from it. Rejecting invalid declarations before generation spend (rather than v2's graceful-degrade) means a hallucinated or malformed dependency costs nothing and fails loudly and specifically, not silently or expensively. Cascading unavailability (section 7) exists because chaining is only safe if a downstream workflow's assumptions about an upstream reference are actually true — building B against a reference to A when A never successfully existed would produce a workflow whose generated content silently assumes something false. The dry-run/unavailable distinction (sections 4, 7, 8) exists because those two conditions look superficially similar (`workflowId: null` either way) but mean opposite things for whether downstream building should proceed. Building in topologically-sorted order eliminates forward-reference bugs by construction. Slug-based keys remove a second, independent source of silent mis-matching. The method/path/URL distinction closes a real correctness gap where a bare path could be mistaken for a live, callable endpoint.

**Process:**
1. Draft `WorkflowReference` (with `deployed` + method/path/URL fields) + `toWorkflowReference()`; write it and its unit tests first — pure, isolated. Tests: `webhookUrl` absent when no base URL supplied; present and correctly constructed when one is; `deployed: false` with `workflowId`/`webhookUrl` both absent for a dry-run `BuildResult`; `deployed: true` with both populated for a deployed one.
2. Draft `resolveBuildOrder()` with unit tests covering, at minimum: no dependencies (trivial order = plan order, nothing rejected); a simple valid chain; a forward-declared-in-plan-but-resolvable dependency (confirms reordering); an unknown-dependency case (confirms per-workflow rejection, not pack-wide escalation); a self-dependency case; a cycle case (2-workflow and 3-workflow) — confirms every one of these three rejection cases produces a `rejected` entry and generation is never attempted for that key; a duplicate-dependency case confirming the *opposite* — a `deduped` entry, but the workflow still appears fully buildable with the collapsed edge set, not blocked.
3. Draft the cascading-unavailability logic (section 7) as its own unit-tested function or clearly separated block: a 3-workflow chain (C→B→A) where A's build throws, confirming B and C are both marked unavailable and neither is attempted; the same chain where A only dry-run-succeeds, confirming B and C both build normally using A's non-deploy-time fields.
4. Draft the workflow-key assignment (slug + dedup suffix) at plan-normalization time, reusing `slugifyWorkflowName`.
5. Draft the `PLAN_PROMPT` addition with a concrete worked example, asking for `dependsOn` by workflow *name*.
6. Draft the `BuildOptions.priorContext` extension and the exact rendered-prompt-section text for a 2-workflow chained example, including the dry-run caveat line from section 6 — sanity-check length and clarity by hand before writing code.
7. Draft the pack-builder loop's accumulation/lookup/gate logic, operating over `resolveBuildOrder()`'s output order and section 7's availability map.
8. Write this design as a short doc; manually trace it through 2–3 realistic multi-workflow business descriptions, **plus explicit pathological cases**: an LLM-declared cycle, a self-dependency, a duplicate dependency, an unknown dependency name, a build-time failure cascading to two downstream workflows, and a fully dry-run 3-workflow chain — confirming each behaves exactly as specified above before moving to Step 8.
...
- Zero behavior change for packs whose plan declares no dependencies — same prompt, same cost, same output, same build order as today.
- Unknown/self/cyclic dependency declarations are always rejected per-workflow, before any generation spend for that workflow — never silently dropped, never built-anyway-with-a-warning (v2's behavior), never escalated pack-wide.
- Duplicate dependencies are always detected and surfaced, never silently dropped — but deduped and built, never rejected; a redundant-but-valid declaration is not the same defect class as unknown/self/cyclic.
- A downstream workflow whose required dependency is genuinely unavailable (rejected or build-failed upstream) does not build or activate — this cascades transitively even though context injection stays direct-only.
...
- `webhookUrl` is never fabricated when `N8N_BASE_URL` is unknown — absence is the correct, honest signal.
...
**What to avoid:** Building a generic "task context" framework or inter-step message bus resembling crewAI's `Task.context` machinery wholesale — the actual mechanism here is one typed struct, one validation/ordering function, one availability-gating map, one rendering function. Heuristic/automatic dependency inference from descriptions — require explicit LLM declaration. Solving transitive *context* (C injecting A's data through B) in this version — direct dependencies only for what gets rendered into a prompt (transitive *unavailability propagation* per section 7 is required regardless, since it's a safety property, not a context-richness feature). Letting a bare webhook path be rendered or treated as if it were a complete URL. Reviving v2's graceful-degrade language anywhere in Step 8 — it is superseded, not an alternative to choose between.

**Outcome / Definition of done:** A written design covering: the `workflowKey` scheme; the `WorkflowReference` schema (with `deployed` + method/path/URL); `resolveBuildOrder()`'s validation/ordering behavior across unknown/self/cycle (rejected) and duplicate (deduped, not rejected — a distinction found and locked during this design pass, not assumed uniform); the per-workflow (not pack-wide) rejection semantics; the cascading build-time availability gate distinct from plan-time rejection; the dry-run/unavailable distinction with its own worked trace; the `PLAN_PROMPT` and `BuildOptions` additions; the token-budget guardrail; backward compatibility for dependency-free plans — manually traced against 2–3 realistic examples plus the explicit pathological cases listed in Process step 8.
...
1. `WorkflowReference` type (with `workflowKey`/`deployed`/`httpMethod`/`webhookPath`/`webhookUrl`) + `toWorkflowReference()` + unit tests: no `webhookUrl` without a known base URL; `deployed: false` with `workflowId`/`webhookUrl` both absent for a dry-run `BuildResult`; `deployed: true` with both populated for a deployed one.
...
3. `resolveBuildOrder()` — dependency-graph validation and topological sort, per-workflow rejection (not pack-wide escalation) — + unit tests: no deps, valid chain, forward-reference reordering, unknown dependency, self-dependency, cycle (2- and 3-workflow), and a duplicate dependency confirming dedup-and-build rather than rejection. Confirms every rejection case (unknown/self/cycle) is scoped to the one affected `workflowKey`, never the whole plan, and that a duplicate never produces a rejection at all. This is the commit most worth getting right in isolation before anything downstream depends on it.
...
5. `BuildOptions.priorContext` extension + `prompt-builder.ts` rendering (with method/path/URL rendered as distinct labeled fields, and an explicit "(not yet deployed)" caveat line for `deployed: false` entries) + unit tests confirming deterministic, compact, exactly-expected-fields output for both deployed and dry-run references.
6. The cascading build-time availability gate (Step 7 §7): a `Map<string, WorkflowReference | 'unavailable'>` built alongside the build loop, downstream workflows skipped (not attempted, no generation spend) when any dependency resolves to `'unavailable'` — + unit tests: a 3-workflow chain where the root build throws, confirming both downstream workflows are skipped and neither is attempted; the same chain fully dry-run, confirming all three build normally.
7. `pack-builder.ts`'s loop change wiring it all together: build in `resolveBuildOrder()`'s order (not plan order), accumulate `WorkflowReference`s keyed by `workflowKey`, look up validated `dependsOn`, pass `priorContext`, apply commit 6's availability gate, set `PackWorkflowResult.error` (not a pack-wide escalation) for both rejected-declaration and unavailable-dependency cases.
8. A new golden pack fixture specifically exercising chaining — two workflows where the second `dependsOn` the first — asserting the second's generated JSON references the first's **real** webhook path/method (not guessed/hallucinated); a fixture confirming a pack with no declared dependencies behaves byte-for-byte as before this step; a fixture confirming an unknown/self/cyclic declaration rejects only the affected workflow, with the rest of the pack unaffected; a fixture confirming a duplicate declaration builds normally against the deduped edge set (not rejected); a fixture confirming a build-time failure cascades to skip its dependents; a fixture confirming a fully-dry-run chained pack builds every workflow normally with `deployed: false` throughout.

**Where:** Same files as Step 7, plus `tests/` for the new chaining fixture(s).

**Reasoning:** Eight small commits behind fixtures (not one large commit) means any regression is caught and attributable to a specific small change — consistent with the one-commit-per-item discipline the Delivery Bundle and preflight arcs already established as Kairos's working pattern. `resolveBuildOrder()` lands before the prompt/loop changes specifically because everything downstream depends on its correctness, and it's the piece most amenable to thorough, fast, isolated unit testing before any LLM-dependent behavior is layered on top. The availability gate (commit 6) lands as its own commit, separate from the loop wiring (commit 7), because it's a distinct safety property (section 7 of the design) from the mechanical accumulate-and-lookup logic, and deserves its own isolated test before being tangled into the loop.

**Process:**
1. Land commit 1; run full suite.
2. Land commit 2; run full suite.
3. Land commit 3; run full suite — this is the commit to scrutinize hardest, since a bug in build ordering, cycle detection, or the duplicate/self cases would silently corrupt every subsequent chained pack.
...
5. Land commit 5; run full suite; manually inspect the actual rendered prompt text for both a deployed and a dry-run 2-workflow chained scenario, confirming method/path/URL read as distinct fields and the dry-run caveat renders correctly.
6. Land commit 6; run full suite — scrutinize this one nearly as hard as commit 3, since an inverted condition here (e.g. treating dry-run as unavailable) would silently break chaining for every dry-run pack, the most common real-world case.
7. Land commit 7; run full suite.
8. Land commit 8; run full suite — this is the commit that actually proves the feature end to end, including every failure-mode and the dry-run case.
9. Measure token delta: tokens-per-build for a chained vs. non-chained pack of similar size, confirming the Step 7 token-budget guardrail holds in practice, not just on paper.
...
**What to avoid:** Skipping straight to assembling the whole mechanism at once — the incremental sequence exists specifically so a subtle bug (e.g., an off-by-one in the topological sort, an inverted dry-run check, or a slightly malformed rendered-prompt section) is caught at the smallest possible unit, before it's tangled up with LLM-dependent behavior that's harder to debug. Reintroducing v2's pack-wide escalation channel for any rejection case here — Step 7 §3 deliberately supersedes that with per-workflow `error`.

**Outcome / Definition of done:** All eight commits land; the chaining fixture passes, demonstrating a correct real cross-workflow reference; the "no dependencies declared → unchanged behavior" fixture also passes; the "unknown/self/cyclic declaration → per-workflow rejection, rest of pack unaffected" fixture also passes; the "duplicate declaration → deduped, builds normally" fixture also passes; the "build-time failure cascades to dependents" fixture also passes; the "fully dry-run chained pack builds normally" fixture also passes; token delta measured and reported; full suite (incl. Steps 2–6a) green; typecheck/lint clean.
...
- [x] Step 7 (design locked, v3 — no code written, per instruction): `workflowKey` assignment (unchanged from v2); `resolveBuildOrder()` now covers seven test cases (no-deps, valid chain, forward-reference, unknown, self, cycle, duplicate) across four validation categories — unknown/self/cycle get **per-workflow rejection before generation spend**, superseding v2's graceful-degrade-with-warning; duplicate turned out, on manual trace-through, to warrant a lighter response (deduped and built, not rejected — a redundant-but-valid declaration isn't the same defect class as the other three); a new build-time cascading-availability gate (§7) distinct from plan-time rejection — a downstream workflow does not build when a validly-declared dependency's upstream build actually failed, and this cascades transitively; an explicit dry-run-vs-unavailable distinction (§8) so a pack-wide dry-run build (confirmed: `dryRun`/`activate` are pack-wide today, no per-workflow mixing) still gets full content-level chaining, honestly missing only deploy-time fields, never blocked. `WorkflowReference` gained a `deployed: boolean` field to make this distinction unambiguous rather than relying on a `workflowId: null` check that can't tell "never deployed" from "failed." Step 8's implementation plan updated in the same pass for consistency (8 commits now, not 7 — the availability gate is its own commit).
- [ ] Step 8: eight chaining commits landed (per Step 7 v3), chaining + no-dependency + rejection-scoped + cascading-failure + fully-dry-run fixtures all passing, token delta measured
