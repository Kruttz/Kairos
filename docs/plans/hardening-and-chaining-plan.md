...
- [x] Step 5 (provenance tuple — 10 commits total across the initial pass + a Codex-reviewed closure pass): `computeWorkflowHash()` with a bumpable schema-version prefix (nodes/connections/settings, 12 tests incl. connection-sensitivity); content-derived `ruleSetVersion`/`promptTemplateVersion`+`promptProfile`/`nodeCatalogVersion`/`kairosVersion` (walks up to the nearest `package.json`, correct in both dev and the flat `dist/` tsup produces); `BuildResult.provenance` with real model/maxTokens/temperature/runId + the above; `PackWorkflowResult.provenance` preserving it through the pack-builder boundary (previously silently discarded — the single biggest gap the review found); `BundleManifest.provenance` (export-time) plus per-workflow `liveExportHash`/`originalBuildHash` (build-time, from `PackWorkflowResult.provenance`) so build-vs-live drift is directly visible, not just export-time version comparison; `PreflightResult.provenance` with a same/different/predates-tracking comparison check including `kairosVersion`/`promptProfile`. A real literal-NUL-byte hygiene bug in `pack-bundle.ts` (unrelated to provenance, found by the same review) fixed in its own no-behavior-change commit. ~50 new/updated tests, 1217/1217 passing overall. Still outstanding, explicitly not silently dropped: the "restore candidate" `handoff.md` wording correction (below), and activation provenance (moved to Step 6b's `workflow_activated` payload, see §7's note — never was implemented under Step 5).
