...
- [ ] Step 1: telemetry findings note written
- [ ] Step 2: 2–3 semantic golden fixtures (stubbed LLM, mocked n8n, zero live calls) green in required CI; benchmark baseline exists as a separate manual artifact, not a required-CI gate; no-network guard verified to actually catch a stray call
- [ ] Step 3: audit report + `audit-node-catalog-parity.ts` script, re-runnable; all repro tests either uncommitted-until-fixed or committed with explicit skip markers, never bare-red on the shared branch
- [ ] Step 4: every confirmed defect fixed with its test landed atomically in the same commit; CHANGELOG updated; resolver import still untaken; stop-and-report threshold re-checked before patching began
- [ ] Step 5: provenance tuple (incl. `connections`/`settings` in the hash) in manifest/preflight/activation; hash function tested for stability, node/parameter-sensitivity, and connection-sensitivity; rule-set/prompt versions content-derived (or CI-enforced); "restore candidate" language corrected
