...
**Both hard questions resolved differently than this section's own original sketch, after re-reading the real code they depend on rather than re-deriving from memory** — Codex explicitly required this before implementation:

- **Active-instance version pinning**: NOT the `checkSlaCompliance()` version-cohort refactor floated above (real, but far bigger than "Item 12 only") — a refusal gate at the one clean choke point that already exists. `kairos contract amend --confirm` computes `instanceCounts.in_progress` (reusing `buildPromiseReportData()`, already-shipped, read-only) and refuses (exit 2, nothing written) when the diff has `hasBreakingChanges: true` and any instance is in-flight, unless `--confirm-breaking-with-active-instances` is also passed. Touches zero lines of `sla-compliance.ts`/`report.ts`.
- **Registration staleness**: confirmed directly against `handleContractCompile` that `PackBuilder.build()` always creates brand-new n8n workflow ids on every `--build` (never patches in place) — meaning *every* recompile, not just amendment-triggered ones, already left old deployed workflows orphaned and unpolled the moment `saveContractWorkflowRegistration()`'s old full-overwrite ran. Fixed by making registration append-only, keyed by `n8nWorkflowId` (never by name): `RegisteredWorkflow` gained `contractVersion`/`status: 'active' | 'retired'`; a fresh compile merges in rather than replaces; `kairos ledger poll`/`kairos watch --contracts` now filter to `status === 'active'` (confirmed both already iterate `registration.workflows` generically, so this was the only change needed); `kairos contract report`'s own unattributed-execution-count sum deliberately stays unfiltered (it's a lifetime count, not a live-poll target). The existing `computeDroppedWorkflows`/`--confirm-registration-drop` guard (Finding 2, 2026-07-20) now flips confirmed drops to `status: 'retired'` instead of just permitting a destructive overwrite — a strictly better behavior than what existed before this item, not just a preserved one.

**Shipped**: `src/promise/diff-types.ts` + `src/promise/diff.ts` (new) — `diffProcessContracts(from, to): ContractDiff`, pure, no I/O, one breaking-change principle applied field-by-field across every array in `ProcessContract` (states/events/transitions/startConditions/terminalOutcomes/sla/expirationRules/evidenceRequirements each get their own keyed-array diff with a documented reason string per change; owners/pauseRules/exceptions are uniformly non-breaking since nothing reads their ids from a ProofLedgerEntry/ExceptionDeskItem, confirmed directly against `ledger.ts`/`exception-desk.ts` rather than assumed). `src/promise/store.ts` gained `ContractVersionRecord`, `amendProcessContract()` (archive-then-save; a no-op archive when there's no prior version), `listContractVersions()`, `loadContractVersion()` — `~/.kairos/contracts/<clientId>/<id>/versions/v<N>.json`, unbounded (unlike `snapshot.ts`'s own capped retention — contract history is explicitly never deleted, a much lower-volume history than workflow-write snapshots). `src/promise/registry.ts` reworked as described above, file-locked (`acquireFileLock`, matching `exception-store.ts`'s own "two expected concurrent writers" reasoning) since `saveContractWorkflowRegistration()` is now a real read-modify-write. `cli.ts` gained `kairos contract versions/diff/amend`, and **both existing save-a-contract paths that could previously destroy a prior version silently** — `contract import --confirm-version-change` and, found and fixed in the same pass rather than left as a known adjacent gap, `contract plan`'s and `contract intake`'s own always-saves-regardless-of-conflict paths — now route through `amendProcessContract()` too, closing the root gap everywhere it existed, not just at the one call site the item's own name suggested.

**A real dead-code finding along the way**: `checkContractVersionConflict()` (the old version-conflict helper `contract import` used) became fully unused once all three of its call sites were rewritten to load the prior contract object directly (needed for archiving, not just its version number) — removed rather than left stubbed, confirmed via lint (`no-unused-vars`) before deleting.

19 new tests in `tests/unit/promise/diff.test.ts` (diff output + breaking/compatible classification across every field category), 8 new tests in `tests/unit/promise/store.test.ts` (version archive, old-version load, multi-amendment history), 6 new tests in `tests/unit/promise/registry.test.ts` (append-only merge, status transitions, active-instance polling), 7 new end-to-end CLI tests in `tests/unit/cli/contract-amend.test.ts` (real subprocess runs against an isolated `$HOME`, matching `cli.test.ts`'s own established idiom — no-write preview, confirmed amend, the breaking+active-instance refusal and its explicit override, the validation gate, the same-contract-id guard) — full suite 1949 → 1985, typecheck/lint/docs-drift clean. No live n8n sandbox checkpoint for this item (nothing here touches n8n execution or the sandbox — confirmed this is consistent with Codex's own test list, which named only local/offline checks).
...
**Built exactly as planned, no scope surprises** — the design held up against real code unchanged, unlike several earlier items in this arc. `src/promise/value-types.ts`/`value-report.ts` — `ImpactAssumptions`/`ValueLineItem`/`AutomationValueReport` types plus `validateImpactAssumptions()` (a separate, pure pre-check — refuses with a clear reason before any computation runs, mirroring `validateProcessContract()`'s own "validate then use" precedent, rather than folding the refusal into the builder itself), `buildAutomationValueReport(reportData, assumptions?)` (pure, wraps `PromiseReportData` unchanged as `observed`, computes an `estimatedValue` section only when at least one real line item results), and `generateAutomationValueReport()` (renders `observed` via the *existing*, unmodified `generatePromiseReport()` — confirmed byte-identical by test — then appends an "Estimated Value" table only when present). `src/promise/report-bundle.ts` gained `writeAutomationValueReport()`, identical idiom to the existing `writePromiseReport()`, writing `automation-value-report.md` as its own separate artifact rather than replacing the existing one. `cli.ts` gained `kairos contract value`, documented inline in all three help-text surfaces (top-level `Usage:`, `handleContract`'s own usage/description text, and the detailed `--help` "Contract options" listing) *as it was built*, not deferred to a later pass — directly applying the lesson from this session's own preceding documentation-currency review, which found items 4–12 had gone undocumented in README/CHANGELOG for an entire arc.

**A real design decision worth naming explicitly**: line items are gated only on whether the corresponding `ImpactAssumptions` field was supplied, never on whether the underlying count is greater than zero. A supplied assumption against a zero count still renders a real "0 x N = 0" line — omitting it when the count happened to be zero would have been a surprising, inconsistent special case, and would have meant the report's own shape silently changed from window to window even though the human's assumptions didn't change. Verified with a dedicated test.

**No live checkpoint needed and none attempted** — this module is pure computation over already-real `PromiseReportData` (the same data `kairos contract report`'s own already-live-checkpointed extraction path produces); nothing here touches n8n, the network, or an LLM.

22 new unit tests (`tests/unit/promise/value-report.test.ts` — covering the "no assumptions → no value figures" invariant, partial-assumption line-item isolation, exact `count × perUnitAssumption` arithmetic for all four assumption fields, the zero-count-still-renders case, and the disclaimer) plus 6 new end-to-end CLI tests (`tests/unit/cli/contract-value.test.ts` — no-assumptions, with-assumptions, the missing-currency refusal, and `--bundle` writing both files) — full suite 2022 → 2050, typecheck/lint/docs-drift clean.
...
**Built narrower than this sketch, on Codex's own restated scope, in two deliberate ways.** First, the CLI is `kairos scout analyze <file> [--out <dir>] [--json]` plus five `--*-column` hint flags — not `scout csv`/`scout to-intake`, and no `--client-id` at all: Scout is not scoped to any saved contract or client in v0, it analyzes exactly the file you point it at. Second, and more significant: **no `scout to-intake` bridge command was built.** `intake.ts`'s existing `--context <file>` flag (re-verified directly against current code before implementation) already reads a file as plain text verbatim, truncated at `MAX_CONTEXT_CHARS`, straight into the LLM synthesis transcript — a human can already hand-copy a finding's own `recommendedNextStep`/`possibleProcessContractSeed` text into a file and pass it to `--context` today, with zero new code. Building a dedicated bridge command would have been scope creep past what Codex actually asked for; the existing flag is the v0 handoff path, by design, not by omission.

**Detects 10 checks, not this sketch's 3** (`STALE_ROWS`, `STUCK_STATUS`, `MISSING_OWNER`, `MISSING_NEXT_ACTION`, `DUPLICATE_RECORDS`, `LONG_GAPS_BETWEEN_TIMESTAMPS`, `UNCLOSED_LOOPS`, `POSSIBLE_HANDOFF_DELAY`, `REPEATED_MANUAL_STATUS_VALUES`, `CANDIDATE_PROCESS_NAME`) per Codex's own restated scope, which named all 10 explicitly. `src/scout/types.ts` — `OpportunitySource`, `ColumnHints`, `ResolvedColumnRole { column, source: 'hint' | 'guessed' }`, `OpportunityFinding` (finding id, checkId, severity, description, sourceFile, evidence row refs, confidence, recommendedNextStep, possibleProcessContractSeed, caveats[]), `OpportunityReport`, `SkippedCheck { checkId, reason }`. `src/scout/csv-source.ts` — a dependency-free, hand-written character-by-character `parseCsv()` state machine (handles quoted fields, embedded commas/newlines, doubled-quote escaping, CRLF) rather than an npm dependency, per the explicit "don't let format support become the project" guardrail; `detectColumnRoles()` resolves all 5 roles independently, hint-first then header-keyword-guess, every resolution tagged with its own `source` so every downstream finding can honestly say whether the column it used was confirmed or guessed. `src/scout/checks.ts` — the 10 detection functions plus `runOpportunityChecks()`; a check whose required column role can't be resolved is skipped with a stated reason rather than run against the wrong column or silently omitted. `src/scout/analyze.ts` — `analyzeCsvFile()`/`generateOpportunityReport()`. `src/scout/report-bundle.ts` — `writeOpportunityReport()`, same `--out <dir>` + manifest idiom as `report-bundle.ts` (promise) and item 13's value-report bundle.

**The no-raw-cell-value guardrail is uniform across all 10 checks, verified by a dedicated test** (`the rendered text output never contains a raw cell value`, using a sentinel string planted in source data) — every finding reports row *indices*, *counts*, and *column names* only, never cell content, with one deliberate, narrow exception: `CANDIDATE_PROCESS_NAME` references column **headers** (schema, not data) to guess a process name, which is a different, non-sensitive category of source-file text.

**No live checkpoint needed and none attempted** — pure local file I/O, no network/n8n/LLM anywhere in this module, consistent with `analyzeCsvFile()`'s own signature taking a file path and an optional injected clock, nothing else.

44 new tests (14 `csv-source.test.ts`, 20 `checks.test.ts`, 4 `analyze.test.ts`, 6 end-to-end CLI `scout.test.ts` using real `spawnSync` subprocess calls against real temp CSV files, matching every other CLI test file's own established idiom) — full suite 2050 → 2094, typecheck/lint/docs-drift clean. One real test-assertion bug found and fixed during this pass: an initial `MIN_ROWS_FOR_REPORT` test asserted every check's skip reason contains the phrase "data rows," but `CANDIDATE_PROCESS_NAME` has its own independent skip reason ("No single significant word...") since it reasons over headers, not row count — fixed by excluding that check's own skip from the shared assertion.
...
**Built to a deliberately narrower, safer design than this section's own original sketch above — Codex's actual v0 authorization superseded it, not an implementation shortcut.** The sketch above already had confirmed patterns feeding `plan.ts`/`intake.ts` synthesis prompts (prompt-context mutation, gated but real). Codex's real scope for this turn was explicit and stricter: *"no automatic prompt changes... Promotion should only mark the note as promoted locally; it should not mutate prompts/rules/code in v0."* v0 as actually built has **zero integration points into `plan.ts`, `intake.ts`, the validator, scenario generation, or the compiler** — `learning.ts`/`learning-store.ts` are never imported by any of those files, and nothing calls into `learning-store.ts` except `handleLearn()` in `cli.ts`. This is a stricter posture than even this section's own original design, appropriate given Codex's own explicit "Kairos changes itself" risk framing for this item.

**Data model is `LearningNote`, not `ContractPattern`** — `src/promise/learning-types.ts` (`LearningNoteDecision`, `LearningNoteStatus: 'candidate' | 'promoted' | 'rejected'`, `LearningNoteStatusChange`, `LearningNoteProvenance`, `LearningNote`). One evidence source only, per Codex's own "focus on accepted/rejected Contract Evolution proposals first": a note is derived 1:1 from one `ContractAmendmentProposal` (item 11) that a human has already decided (`accepted`/`applied`/`rejected` — `applied` counts as `decision: 'accepted'`, being strictly stronger evidence than merely `accepted`). A still-`proposed` proposal produces nothing — `src/promise/learning.ts`'s `deriveLearningNotesFromProposals()` is pure, deterministic, no I/O, mirroring `evolution.ts`'s own shape exactly.

**The one scope decision made, not fully covered by Codex's own most literal wording**: v0 builds notes ONLY from Contract Evolution proposal decisions, not directly from raw repeated ProofLedger/ExceptionDesk/Promise Report patterns (named as optional — "if already easy to read"). Every one of those evidence sources already flows INTO a Contract Evolution proposal today; a second, parallel extraction straight from raw evidence would either duplicate `evolution.ts`'s own detection logic or risk a "learning" the corresponding proposal doesn't already reflect, undermining Codex's own "Item 11 produces proposals, Item 15 records decisions about them" framing. Deferred, not rejected.

**The structural synthetic-evidence guardrail** (the single most important test in this item, per Codex's own framing): `LearningNoteProvenance.synthetic` is never a human- or proposal-settable field — computed fresh every time from the note's own copied `evidence` array as `evidence.length === 0 || evidence.every(e => e.kind === 'harness_scenario')` (fail-closed: an evidence-less note is also treated as unpromotable, though this can't happen today given `evolution.ts`'s own "evidence never empty" invariant). `updateLearningNoteStatus()` in `learning-store.ts` is the one enforcement point for the whole module — every caller, CLI included, goes through it — and it throws `SyntheticNotePromotionError` *before* any write happens when `to === 'promoted'` and `provenance.synthetic` is true, verified by a dedicated test confirming the refused note is byte-identical on disk afterward (still `'candidate'`, empty history).

**Storage is scoped to the CLIENT, not one contract** — a deliberate, and non-obvious, divergence from `evolution-store.ts`'s own per-contract `amendment-proposals.json`. `~/.kairos/promise-ledger/<clientId>/learning-notes.json` is the first client-level (not contract-level) file in that directory tree, confirmed empty/unused at that level before choosing the path — a learning is a client-level fact (`kairos learn list --client-id <slug>` intentionally has no `<contract-id>` argument), never a cross-client one; there is no code path anywhere in this module that reads more than one clientId's directory in a single call. `upsertLearningNotes()` mirrors `upsertContractAmendmentProposals()`'s exact upsert-by-deterministic-id reasoning: a note id is `${clientId}-${contractId}-${proposalId}` (not including the decision), so re-running `learn candidates` refreshes the same note's `summary`/`recommendedNextAction`/`provenance` while preserving a human's own prior `status`/`history`/`createdAt` on that note — the direct answer to "duplicate candidate notes are deduped or preserved intentionally."

**CLI**: `kairos learn candidates <contract-id> --client-id <slug>` (generate/refresh), `learn list --client-id <slug> [--status <status>]`, `learn show <note-id>`, `learn promote <note-id> [--reason <text>]`, `learn reject <note-id> --reason <text>` (reason required on reject, matching `contract evolve reject`'s own convention but stricter — enforced in `cli.ts`, not just documented). Fully standalone, off-by-default — a human must explicitly run `learn candidates`; nothing auto-triggers from `contract evolve accept/reject`.

**No live checkpoint needed and none attempted** — pure local file I/O, no network/n8n/LLM anywhere in this module. Manually verified end-to-end against a real drifting-ledger fixture (accept → `learn candidates` → `learn promote`) to confirm rendered CLI output quality before considering this done.

43 new tests: 12 `learning.test.ts` (pure derivation, including the fail-closed empty-evidence case and a defensive mixed-evidence case `evolution.ts` never actually produces today), 18 `learning-store.test.ts` (upsert/dedupe/preserve-status, client isolation, the synthetic-promotion guardrail including the "refusal writes nothing" case, and confirming reject is *not* blocked by the synthetic guardrail — only promote is), 13 end-to-end CLI `learn.test.ts` (real `spawnSync` subprocess chains through `contract import` → `contract evolve run/accept/reject` → `learn candidates/promote/reject`, including a live "the source contract is byte-identical before/after promote" assertion) — full suite 2094 → 2137, typecheck/lint/docs-drift clean.
