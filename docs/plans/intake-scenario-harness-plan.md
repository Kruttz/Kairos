...
**Same architectural constraint Phase 7 found, re-confirmed against `compile.ts` for this phase rather than assumed carried over**: `buildIntakeWorkflow()` only ever handles a contract's `StartCondition`s; `buildProcessingWorkflow()` owns every `ProcessTransition`, including the ones evidencing a failure terminal. A chaos run injects payloads at ONE workflow's webhook — for the intake workflow, that means only `instance_start`-shaped evidence is producible/checkable at all, exactly as Phase 7 found for replay.

**Category coverage was decided against that constraint, not against this section's own original sketch above** — the original `missing_data`/`after_hours` definition-of-done target turned out to be the two categories LEAST supportable by a live injection, for two different, real reasons found while designing against current code: `after_hours` is a wall-clock property of when the sandbox actually executes, not anything a payload's own content can carry (a live injection cannot backdate itself into the past the way the purely-synthetic Scenario Generator can); and the harness's own `missing_data` targets a Processing-stage `EvidenceRequirement`, unreachable from an intake-only injection for the same reason `failure_terminal` is. Of Codex's full named list (missing data, duplicate/correlation ambiguity, late/no response, after-hours, failure terminal, in-progress), three survive as real, honest, intake-testable chaos categories, each attempted for real rather than guessed at:
- **`happy_path`** — a complete, valid intake payload; also this run's own crash-classification reference.
- **`missing_correlation_key`** (new, chaos-specific — deliberately NOT the harness's own `missing_data`, to avoid the two ever being confused) — the raw intake payload with `contract.correlationKey.fieldPath` removed entirely, probing whether the intake workflow fabricates or mis-attributes an `instance_start` when it has nothing real to key by.
- **`duplicate_correlation`** — the same payload injected twice, close together, reusing `generateContractScenarios()`'s own `duplicate_correlation` category (which already models exactly two `instance_start` events under one correlation key) unchanged.

`failure_terminal`, `no_response`, `after_hours`, and `in_progress` are explicitly skipped, each with its own honest, specific reason (not a generic "not supported"), returned in every run's own `skipped: ContractChaosSkip[]` — mirroring `ScenarioGenerationSkip`'s established pattern rather than silently omitting them.

**Shipped**: `src/reliability/chaos/contract-outcome.ts` (new, named to mirror `reliability/replay/contract-outcome.ts` deliberately) — `generateContractChaosVariants()` (pure, deterministic, reuses `generateContractScenarios()` + `scenarioIntakePayloadBody()` unchanged rather than re-deriving payload construction), `evaluateContractChaosOutcome()` (pure comparison, branches on each variant's own expectation — `expectNoInstanceStart` for missing_correlation_key, `injectTwice`/exactly-2 for duplicate_correlation, default single-match for happy_path — since chaos's own definition of "correct" differs by category, unlike replay's simpler "should always match"), and `runContractChaos()` (the sandbox-owning orchestrator: import once, inject every variant against the SAME imported workflow with happy_path always processed first so its execution is available as the crash-classification reference for the rest, extract real evidence via `extractExecutionEvidence()`, compare, always clean up). `ContractChaosVariant extends ChaosPayloadVariant` unchanged (`payloads.ts`'s own `{name, rationale, body}` shape), so this output composes with the existing chaos type by construction rather than a parallel type needing its own handling — the reuse discipline this section's own original design note called for. Crash-classification is genuinely dual-signal, not new logic: each non-happy-path variant's execution is diffed against the happy_path reference via `diffPayloadExecution()`/`classifyChaosPayloadDiff()`, both reused completely unchanged from `sandbox-run.ts`. `kairos chaos run <id> --contract <file.json>` wired into `cli.ts` as a genuinely separate, additive block after the existing structural `runChaosSandbox()` call (never replacing it, per Codex's explicit guardrail) — its own "contract-derived chaos" console section, folded into the combined exit code (a business-outcome mismatch fails the command exactly like a confirmed crash does, matching `kairos replay run --contract`'s own precedent).

**Module boundary**: a new, narrow allow-list block added to `tests/unit/reliability/module-boundaries.test.ts`, mirroring the `reliability/replay/` one exactly with one addition — `reliability/chaos/` may import `promise/ledger.js`, `promise/ledger-types.js`, `promise/types.js`, `promise/scenario-types.js`, AND `promise/scenario.js` (this module calls `generateContractScenarios()` itself, unlike replay's `contract-outcome.ts`, which only ever consumed already-generated `ContractScenario` values) — and is asserted to never import the file-backed persistence layer or reference a promise-engine storage path literally, for the same reason replay's own boundary exists.

**No new production bug found this pass** — worth stating plainly rather than manufacturing a finding to match Phase 7's own precedent: the live checkpoint's passing run matched on the first attempt, for all 3 categories, including the two-real-injections `duplicate_correlation` case. The one real design correction this phase made was caught during design, not live testing: `runContractChaos()` gained an explicit `variantsOverride` parameter (mirroring `checkScenarioIntakeOutcome()`'s own explicit `scenario` parameter, Phase 7) specifically so a live checkpoint could prove a genuine mismatch — real injections, real executions, real extracted evidence — against a deliberately-wrong expectation, without needing a fixture contract that is actually broken. Never used by the CLI, which always lets the contract itself decide its own variants.

**Live-checkpointed against a real local n8n sandbox** (never production) with a bare hand-built webhook-trigger workflow (`settings: {}` included from the start this time, applying Phase 7's own checkpoint-script gotcha rather than rediscovering it): a passing run where all 3 categories matched real sandbox executions — `happy_path` and `missing_correlation_key` each producing exactly the evidence their own category expects (one real `instance_start`, and genuinely none, respectively), `duplicate_correlation` producing two real `instance_start` entries under one real correlation key hash from two real injections, with `crashClassification: HANDLED` on both non-reference variants; a `not_webhook_shaped` case requiring no network call; and a mismatch run (same real workflow, same real injections/executions) where all 3 categories were deliberately given wrong expectations via `variantsOverride` — a wrong `expectedInitialState`, a `missing_correlation_key` variant wrongly expecting a match, and a `duplicate_correlation` variant with a tampered `correlationKeyValue` — and every one was correctly flagged as a mismatch against genuinely real, not synthetic, extracted evidence.

19 new unit tests (`tests/unit/reliability/chaos/contract-outcome.test.ts`, covering variant generation across all 3 fixtures including the two where `happy_path` is itself skipped, both pure comparators, and the no-network-call statuses) plus 3 new module-boundary tests, full suite 1927 → 1949, typecheck/lint/docs-drift clean.
