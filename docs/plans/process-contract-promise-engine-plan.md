...
**Status: Phase 0 SHIPPED 2026-07-19 (schema + deterministic validator + minimal storage). Phases 1-5 remain planning only — not built, not started.** Per Codex's explicit approval: *"Plan reviewed. Direction approved. Start Phase 0 only... Do not implement Phase 1-5 yet."* Phase 0's own scope was pressure-tested against a second, deliberately different example before any type was locked in (§4.5b), and two more real, previously-hidden gaps (`StartCondition.initialState`; `ExpirationRule` edges missing from the reachability check) were found while actually implementing the validator against real fixtures, not while designing it on paper — both fixed and recorded where they were found, not folded silently into the original text. See §10's Phase 0 entry for the full build record.
