...
**Status: PLANNING ONLY. No code has been written for any part of this plan.** Per Codex's explicit instruction: *"Let's start the next arc now, but planning/design only first — no implementation until the plan is reviewed."* This document is a design-verification pass against the actual current codebase (not a fresh sketch) plus an architecture proposal, written for review before a single line of `ProcessContract`/`ProofLedger`/`ExceptionDesk` code exists.
...
- `kairos contract plan "<business description>"` CLI command.
- **Checkpoint:** a real-model spot check against a plain-language version of the Empire Homecare promise (not the pre-structured §4.5 form — the actual test is whether the LLM authoring step can *produce* something close to §4.5 from a paragraph, not whether the pre-written version validates). Confirm the drafted contract passes Phase 0's validator and a human reviewer judges it a reasonable reading of the input description.
