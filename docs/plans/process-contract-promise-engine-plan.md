...
- **A real, pre-existing bug found and fixed via this phase's own checkpoint, not predicted:** `getKairosVersion()` (`src/validation/provenance-versions.ts`) reported `'unknown'` when run through the actual bundled `dist/cli.cjs` -- tsup's CJS build shims `import.meta` to a bare `{}`, so `import.meta.url` is `undefined` there, the same class of dist-only bug as the reliability-suite closeout's eager `@anthropic-ai/sdk` import (invisible to any source-tree test, since tests never execute the bundled output). Pre-existing, not introduced by this phase -- `pack-bundle.ts`'s own `bundle-manifest.json` has called the identical function since it shipped and has silently carried the same bug in every real `kairos pack export --bundle` run. Fixed with a `process.argv[1]` fallback, plus a real hardening (`findKairosPackageJson` now verifies `name === '@kairos-sdk/core'` on any candidate, closing a latent gap the new fallback could otherwise have made worse for a CJS library consumer). Both call sites benefit from one fix.
