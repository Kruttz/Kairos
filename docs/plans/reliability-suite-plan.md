...
3. `kairos chaos audit <wf|pack>` → report with the webhook-schema DISCLAIMER discipline (static analysis of expressions is heuristic; says so) — pending (CLI wiring not yet built).
