...
### 5.2 Build: payload capture (`replay/capture.ts`)

1. **Opt-in only:** `kairos replay capture <wf>` (explicit command) — no flag on existing commands silently changes the privacy posture. Config note in README's privacy section.
2. Fetch recent executions (`getExecutions` + `getExecution`), extract the **trigger node's output items** (the workflow's real input) — not every node's data; that's what replay needs and it minimizes stored PII.
3. Store under `~/.kairos/captures/<clientId>/<workflowId>/<executionId>.json`, `chmod 600`, with recorded metadata (executionId, capturedAt, trigger node name, item count).
4. Retention: default cap 20 payloads / 30 days per workflow (configurable); sweep on each capture invocation — no background process.
5. Offer `--scrub` passing captures through the existing memory secret scrubber (best-effort, labeled as such — payloads may still contain PII; docs say so plainly).
6. **Firewall (G4):** the community whitelist serializer (Phase 5) is constructed so captures are not reachable from the share path — different module, no import path, and the whitelist can only emit enum/count fields by type.

### 5.3 Build: sandbox manager (`sandbox/manager.ts` + `transforms.ts`)

1. `sandbox.json` holds url/key/marker; `manager.ts` refuses to treat any URL matching production `N8N_BASE_URL` as a sandbox (G1, enforced in code, tested).
2. Import path: take a workflow JSON → `transforms.ts` applies: name prefix `[kairos-sandbox]`, strip credential bindings (sandbox has none anyway — makes the degraded state explicit rather than accidental), optional trigger-swap (S3b) → create via API on the sandbox instance.
3. Cleanup: `kairos sandbox down` / post-run sweeps delete `[kairos-sandbox]`-prefixed workflows only (prefix is the safety rail against deleting anything else).
4. Test-infra: extend the unit-test mock server patterns to cover executions endpoints, so Phases 1-4 have deterministic fixtures without a live sandbox.
...
3. **Warmup honesty:** below a minimum window (default 5 traces), every check reports `INSUFFICIENT_DATA`, never pass/fail (C6).
...
### 6.4 Surface
- `kairos drift check <wf> [--live]` — offline uses stored traces; `--live` runs `fetchLatestTrace` first. Verdicts: HEALTHY / DRIFTING / INSUFFICIENT_DATA; exit codes 0/1/0.
- MCP `kairos_drift_check`. New telemetry event `drift_check_completed` (schema v3→v4, guarded emission like 0.11.0's ledger).
- Every DRIFTING result appends to `reliability-audit.jsonl` (G6).
...
### 7.2 Runner (`replay/runner.ts`)
1. Resolve baseline (current live JSON or snapshot) and candidate (file / pack version / proposed repair).
2. Import both via sandbox manager (prefix, cred-strip, trigger-swap as decided in S3).
3. For each captured payload: inject → wait for execution (poll `getExecutions` on the sandbox) → `parseExecutionTrace` + capture per-node output **shapes** (keys/types/counts — reusing S1's verified data walk).
4. Cleanup sandbox workflows (prefix-scoped) even on failure paths.

### 7.3 Diff engine (`replay/diff.ts`)
Per payload, compare baseline-run vs candidate-run on: node coverage (which ran), branch path, per-node output shape (keys/types/item counts — not values), error classes, relative duration. Volatile-field list from S4 applied and **labeled heuristic**. Per-payload verdict → suite verdict:
- `IDENTICAL` — no differences beyond volatile fields
- `BENIGN_VARIANCE` — differences only in volatile/duration bands
- `BEHAVIORAL_CHANGE` — branch flips, node coverage change, output-shape change (itemized per node)
- `BROKEN` — candidate errors where baseline didn't

**Verification boundary (Codex second opinion, folded in — required on every replay and chaos report, not optional):** alongside the verdict, every report carries a structured `verificationBoundary` object: `{ verified: [...nodes/paths Kairos actually observed behave, with the evidence], unverifiable: [...nodes/paths Kairos could not observe, each with a reason — credential-blocked, no mock sink for this external call, sandbox lacks X] }`. This is stronger than scattering heuristic labels through free text (the prior draft's approach) — it makes "what did Kairos actually prove, and where did proof stop" a first-class, always-present field on the output type, not something a reader has to infer from disclaimers. `chaos/sandbox-run.ts`'s `BLOCKED_AT_CREDENTIAL` (§9.2) populates this same structure rather than its own separate concept.
