...
4. `kairos chaos run <wf>` — pending (CLI wiring not yet built, next step). Telemetry `chaos_completed` and pattern-system feed also pending, wired at CLI time to match how `drift`/`replay` emit telemetry from their CLI handler, not their underlying module.
