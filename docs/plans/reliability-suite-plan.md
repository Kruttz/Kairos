...
## 10. Phase 5 — Community pattern library (#2) (5a SHIPPED 2026-07-19; 5b explicitly not started — this section re-verified against actual current code before any code was written, per the same G7 discipline as every prior phase, and rewritten with Jordan's explicit conservative-scope guardrail list folded in verbatim)
...
### 10.4 Ingestion (`community/ingest.ts`) — planned only, not built this pass, kept here so 5a's design doesn't need retrofitting later

1. Reviewed submissions merge into `community-patterns.json`, shipped inside the npm package (distribution = normal releases; `kairos patterns sync` = optional raw-GitHub fetch between releases, same file, content-hash-checked).
2. Per Finding 4: ingested patterns live in their **own** file (`~/.kairos/community-patterns.json`), carrying `provenance: 'community'` and a source-repo occurrence count, displayed as read-only annotative context next to local patterns (e.g. *"Rule 42 — also reported by 3 community installs"*) — **never merged into `PatternAnalyzer`'s `Pattern[]`, never an input to `computeCompositeScore()`, never capable of changing a local pattern's `state`.** "Corroboration" means a local `Pattern` for that same rule number already independently exists at `confirmed` or better from this install's own telemetry — at which point the community annotation is surfaced alongside it. A community hint with zero local corroboration is displayed, if at all, clearly labeled as unconfirmed-elsewhere and never influences anything this install trusts.
3. Honest cold-start note in docs: initially the corpus is one maintainer's data; the value compounds with adoption.

### 10.5 Checkpoint (5a scope)

Generate a share report from real local patterns (or, if none are currently `confirmed` on this dev machine, a realistic local fixture standing in for one) → verify by eye that nothing beyond rule/stage/count/confidence appears anywhere in the printed bytes or the written file → confirm declining at the y/N prompt writes nothing and calls nothing → confirm accepting writes `pattern-report.json` and either invokes `gh` or prints a manual URL, never both silently. 5b's ingestion round-trip checkpoint is deferred to whenever 5b actually starts.
...
### 10.7 Build sequence (one commit per piece, matching every prior phase's discipline)

1. Whitelist serializer (`whitelist.ts`) + tests — pure, no network, no filesystem beyond reading `patterns.json`.
2. Extend `module-boundaries.test.ts` to also cover `memory/` (Finding 5) — small, mechanical, lands before `share.ts` so the boundary is enforced the instant real community/ files start importing things, not retrofitted after.
3. Share flow (`share.ts`) + `kairos patterns share` CLI wiring + tests for the pure report-building/filtering logic.
4. Live checkpoint: real (or realistic-fixture) report round-trip, confirm/decline paths, `gh`-present and `gh`-absent paths.
5. Plan doc updated with real findings (this section) — done as the first commit of this sequence, per the same "plan before code" discipline Phase 3 used, rather than only at the end.

**Outcomes / Definition of done (5a):** `kairos patterns share` produces a human-reviewable, provably whitelisted report (confirmed via the type system, not a runtime scrub) and requires explicit, consequence-naming confirmation before anything leaves the machine; no code path in `community/` can reach `captures/` or `memory/` (enforced by an updated, no-longer-vacuous module-boundary test); no network call exists outside the confirmed `gh` spawn; full test suite green, typecheck/lint/docs-drift clean, one commit per step, checkpointed against real or realistic local pattern data. 5b (ingestion) stays explicitly unbuilt, fully specified in §10.4 so 5a's types don't need to change when it eventually starts.
...
P0 → P1 → P2 → P4 → P6 → P3 → P5
     (shipped 2026-07-19, all six)             ^
                                                └─ community pattern library —
                                                   position unchanged, last phase
                                                   in the arc. 5a (export/share)
                                                   builds now; 5b (ingestion)
                                                   explicitly gated on several
                                                   real 5a cycles happening first.
```

P0/P1/P2/P4/P6/P3 are all done — see each section above for exact commit history and live-checkpoint findings. **P5 (sub-phase 5a) builds next — the last remaining phase.** (P4 Tier A floated earlier than a strict P0→P1→P2→P3→P4 reading would suggest, per the original plan's own allowance for it — it had zero dependencies and served as a quick, self-contained win; that flexibility is preserved going forward for any future phase that turns out to have zero real dependencies on its notionally-earlier neighbors.)

**Sizing against demonstrated velocity** (memory layer ~1 session; Delivery Bundle 6 commits/~2 days; preflight 4 phases/2 days; 0.11 chaining 10 commits; **one single session shipped P0+P1+P2 recap, all of P4 (2 tiers, 5 commits, 3 live sandbox checkpoints), all of P6 (4 commits, 4 live checkpoints), and all of P3 (8 commits, live checkpoint, two real latent bugs found and fixed)** — considerably faster than this section's original per-phase estimates assumed):
- P0: shipped · P1: shipped · P2: shipped · P4: shipped (5 commits, both tiers) · P6: shipped (4 commits) · P3: shipped (8 commits, self-healing loop, first phase to write to a live workflow autonomously)
- **P5 (next): 5a in this session; 5b explicitly not scoped by session count** — 5a is smaller and lower-risk than P3 (read-only local aggregation + opt-in export, no live writes), but 5b's own gate ("several real 5a export cycles happened and been reviewed") means it isn't a schedulable next session at all — it starts whenever that real-world precondition is met, not on a calendar.
- **Arc total remaining after this session: 5b only, explicitly gated and unscheduled.** Every phase boundary is a safe pause; with 5a shipped, the arc's only genuinely open item is a precondition-gated sub-phase with no live-write risk of its own.
