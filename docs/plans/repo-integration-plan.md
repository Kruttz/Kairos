...
**Status:** ALL PHASES (1–4) COMPLETE, ALL TRACKED JUDGMENT CALLS RESOLVED (2026-07-02, implemented by Fable 5 — this plan was executed in the same session it was approved in, ahead of the original hand-off-to-Opus-4.8 intent; noted here for continuity). Phase 0 decisions locked (§5). All five Phase 1 amendments implemented (§6). Phase 2 library-scaling work done and measured with real numbers (§7). Phase 3's benchmark actually ran against real API spend (Jordan's explicit go-ahead) — see §8 implementation notes for the honest result (a ceiling effect, not the improvement the old README claimed). The three items tracked after Phase 1/3 (MAX_LIBRARY_SIZE default, CLI test isolation, benchmark tier selection) were planned in §8a and all resolved — see the "Deferred follow-ups tracker" after §6 for commit references. Phase 4's n8n-skills gap analysis is done — see §9 for the findings and the 2 new validator rules (127, 128) + 2 new sub-patterns (`webhook-body-access`, `binary-data-handling`) that came out of it. All phases committed to git (commit `f6e0e3b` is the latest as of this note); nothing has been pushed to GitHub. No further phases are planned in this document — any future work here should start a fresh plan.
...
### Phase 4 judgment calls — OPEN, tracked for after the remaining phases (2026-07-02)

Retroactively logged per Jordan's instruction to keep tracking judgment calls for every phase, not just Phase 1's. Neither of these blocked shipping Phase 4 — both are genuine, deliberate "good enough for now, revisit later if warranted" calls, not oversights.

1. **The ~9 "other domains" got a lighter-touch scan than the 5 priority candidates.** The 5 candidates in the table above were verified by direct grep against `v1.ts`/`validator.ts`/`sub-patterns.ts` line by line. The remaining ~9 n8n-skills domains (MCP Tools Expert, Multi-Instance, Self-Hosting, Validation Expert, Node Configuration, Workflow Patterns, AI Agents, Code Tool, general Error Handling) got a lighter, targeted-grep pass (e.g. confirmed `toolCode` has 2 dedicated rules + catalog + sub-pattern coverage) rather than the same exhaustive treatment. Reasonably confident nothing major was missed, but not to the same evidentiary bar as the 5 that got new rules/patterns. **Revisit:** a full line-by-line pass over the remaining domains, same rigor as the priority 5, if there's ever reason to believe coverage is thinner than assessed here.
2. **Sub-patterns instead of validator rules for `webhook-body-access` and `binary-data-handling` was a deliberate safety call, not a permanent verdict.** Both were kept guidance-only specifically because a static rule risked false positives in both directions (webhook body nesting depends on response-mode/content-type config the validator can't see; binary property names vary legitimately by node type). **Revisit:** if real telemetry ever shows either failure mode manifesting in actual builds despite the prompt guidance (i.e. the sub-pattern isn't sufficient on its own), reconsider a narrower, safer validator rule at that point — don't add one preemptively without that evidence.
