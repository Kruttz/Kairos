...
**Status:** ALL PHASES (1–4) COMPLETE, PHASE 5 NARROW SCOPE SHIPPED, BOTH PHASE 4 JUDGMENT CALLS RESOLVED (updated 2026-07-02, implemented by Fable 5 — this plan was executed in the same session it was approved in, ahead of the original hand-off-to-Opus-4.8 intent; noted here for continuity). Phase 0 decisions locked (§5). All five Phase 1 amendments implemented (§6). Phase 2 library-scaling work done and measured with real numbers (§7). Phase 3's benchmark actually ran against real API spend (Jordan's explicit go-ahead) — see §8 implementation notes for the honest result (a ceiling effect, not the improvement the old README claimed). The three items tracked after Phase 1/3 (MAX_LIBRARY_SIZE default, CLI test isolation, benchmark tier selection) were planned in §8a and all resolved. Phase 4's n8n-skills gap analysis is done (§9) — 5 validator rules total came out of Phase 4 work (127, 128, 130) plus 2 sub-patterns (`webhook-body-access`, `binary-data-handling`); its two tracked judgment calls (§9's tracker) are both resolved as of this update — the domain rescan found and fixed two real correctness bugs (Rule 56/128 reading `onError` from the wrong location, Rule 57 checking the wrong HTTP Request parameter name), and the sub-pattern-vs-rule call resolved as *partially* rule-able (Rule 130 for AWS S3/Slack) with the rest correctly staying sub-pattern-only. Phase 5 (§10) shipped its narrow scope (generated node resource/operation catalog, Rule 129) — the full-scope `displayOptions` conditional-resolver remains an open, explicitly-deferred follow-on Jordan wants to discuss next. Rule count as of this update: 130. All phases committed to git (commit `7fa5d3e` is the latest as of this note); nothing has been pushed to GitHub.
...
### Phase 4 judgment calls (2026-07-02)

Retroactively logged per Jordan's instruction to keep tracking judgment calls for every phase, not just Phase 1's. Neither blocked shipping Phase 4 originally — both were genuine, deliberate "good enough for now, revisit later if warranted" calls, not oversights.

#### 1. RESOLVED — full-rigor rescan of the ~9 non-priority domains

Same bar as the original 5 candidates: direct grep verification against `v1.ts`/`validator.ts`/`sub-patterns.ts`/`workflow.ts`, not a lighter pass.

**Confirmed genuinely covered or out of scope (no action):**
- **Validation Expert** — Kairos's own validator (3387 lines, 127 rules) + `rule-metadata.ts` (529 lines of mitigations/examples/pipeline-stage tags) is a purpose-built superset: n8n-skills' domain is about *interpreting* n8n's own validation errors after the fact; Kairos *prevents* most of them pre-deploy instead.
- **Workflow Patterns** — `src/library/intent-map.ts`'s 7 intents (ai_processing, notification_alert, data_extraction, data_sync, approval_human, scheduled_report, webhook_handler) is a superset of n8n-skills' 5 architectural patterns (webhook, HTTP API, database, AI, scheduled).
- **AI Agents / RAG** — Rules 89 (chainRetrievalQa missing `ai_retriever`) and 97 (vectorStore missing `ai_embedding`) already enforce correct RAG wiring; not in the node-catalog prompt text, but structurally validated, which is the more reliable mechanism anyway.
- **Code Tool** — `toolCode` has Rules 73/94, node-catalog coverage, and is in the `ai-agent-tool-wiring` sub-pattern's `nodeTypes` list. Already confirmed in the original pass; re-confirmed here.
- **Node Configuration** — resource locator (`__rl`) format is explicitly documented in the prompt (lines 182-184) with examples; `fixedCollection`/`multipleValues` (the abstract n8n property-type taxonomy) have zero mentions, but that's an abstraction-level mismatch, not a gap — Kairos already gives the LLM the *concrete* per-node parameter shape (e.g. `assignments.assignments`, `conditions.conditions`) rather than the underlying type-system name for it. Not pursued.
- **MCP Tools Expert, Multi-Instance, Self-Hosting** — zero overlap found (grepped for queue mode / multi-instance / self-host / docker). Confirmed: these are about *using* n8n-mcp or deploying/running n8n itself, not Kairos's generation-quality concern.

**Found genuinely missing, not previously caught (both fixed this pass):**
- **`onError` read from the wrong location — a real correctness bug, not a coverage gap.** Verified against the actual `n8n-workflow` package's `INode` interface (installed as a Phase 5 devDependency): `onError` is a **top-level node field**, sibling to `parameters`. Rule 56 (pre-existing, predates this session) and Rule 128 (shipped in the prior Phase 4 commit) both read `node.parameters['onError']` — meaning neither rule could ever fire against a correctly-shaped real n8n workflow. Fixed in commit `9c6dad0`: added the missing `onError` field to `N8nNode`, corrected both rules to read `node.onError`, added explicit prompt guidance (item 31) since the prompt previously said nothing about where these fields belong, fixed all 5 affected test fixtures (which had encoded the same wrong assumption), and added a regression test.
- **`retryOnFail`/`maxTries`/`waitBetweenTries` had zero coverage anywhere.** Not a correctness bug (no wrong values possible, nothing to structurally validate) — a missing-capability-awareness gap: the LLM had no way to know node-level automatic retry exists, so it would never proactively suggest it for a flaky-API scenario. Fixed in commit `7c9545a` with one prompt guidance line (item 32) — right-sized for what's actually missing, not a sub-pattern or rule.

**Verdict:** the rescan surfaced one real bug (higher-value than anything the original lighter-touch pass caught) and one small legitimate gap, both fixed. Everything else in the ~9 domains checked out. This item is closed — not because nothing was found, but because what was found got fixed rather than re-deferred.

#### 2. PARTIALLY RESOLVED — sub-patterns vs. validator rules for `webhook-body-access` / `binary-data-handling`

While fixing Rule 57 (found during judgment call #1's rescan — it checked the wrong parameter name for HTTP Request binary uploads, see above), the corrected version needed re-verifying against the real n8n-nodes-base package anyway. That check surfaced genuine new evidence relevant to this judgment call, so it got resolved with real information instead of re-deferred by default:

- **`webhook-body-access` — confirmed still correctly sub-pattern-only, no change.** The reasoning holds: `$json.body.field` vs `$json.field` correctness depends on webhook response-mode/content-type configuration the validator can't reliably see, and a downstream Set/Code node commonly and legitimately remaps fields to the top level anyway (a rule flagging "non-body/header/query top-level access after a webhook" would false-positive on that extremely common, correct pattern). Not rule-able.
- **`binary-data-handling` — genuinely resolvable in part, shipped as Rule 130 (commit `7fa5d3e`).** Checked AWS S3 and Slack's real compiled node definitions: both use the identical "toggle + required string property name" shape Rule 57 already safely checks for HTTP Request (unlike the *general* binary-data domain, which still has the same false-positive risk as webhook-body-access — property names vary legitimately across node types with no toggle to key off). This specific slice — S3/Slack file uploads with the binary toggle on and an empty property name — is safely, narrowly checkable, and now is. The sub-pattern stays for the broader guidance (which nodes exist, which parameter name applies per node/typeVersion, the `$binary` object shape) that a rule still can't cover.

**Verdict:** not every gap in a domain resolves the same way — some slices of "binary data handling" were safely rule-able once actually checked against ground truth, others weren't and still aren't. The original sub-pattern-only call wasn't wrong, it was just coarser-grained than the evidence turned out to support. Both parts now reflect what's actually true, not a blanket "possible" or "not possible" applied to the whole domain.
